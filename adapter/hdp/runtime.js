...
const SENSOR_VALUE_KEYS = new Set([
  'temperature_millicelsius', 'humidity_millipercent', 'pressure_pascal',
  'illuminance_millilux', 'bus_voltage_microvolts', 'shunt_voltage_microvolts',
  'current_microamps', 'power_microwatts', 'raw', 'distance_millimeters',
]);

function validSensorSample(payload) {
  return payload && typeof payload.sensor_id === 'string'
    && /^[A-Za-z0-9._-]{1,16}$/.test(payload.sensor_id)
    && typeof payload.sensor_type === 'string'
    && uint32(payload.sample_sequence)
    && uint32(payload.captured_at_uptime_milliseconds)
    && ['ok', 'error'].includes(payload.status)
    && ((payload.status === 'ok' && payload.error === null)
      || (payload.status === 'error' && typeof payload.error === 'string'))
    && payload.values && typeof payload.values === 'object' && !Array.isArray(payload.values)
    && Object.entries(payload.values).every(([key, value]) =>
      SENSOR_VALUE_KEYS.has(key) && Number.isInteger(value)
      && value >= -2147483648 && value <= 2147483647);
}

function validSensorMessage(message) {
  const payload = message && message.payload;
  if (!payload || !uint32(payload.config_revision)) return false;
  if (message.type === 'sensor.sample') return validSensorSample(payload);
  if (message.type === 'sensor.status') {
    return typeof payload.reply_to === 'string'
      && uint32(payload.captured_at_uptime_milliseconds)
      && Array.isArray(payload.samples) && payload.samples.every(validSensorSample);
  }
  return false;
}
...
class RuntimeConnection extends EventEmitter {
  ...
  onMessage(raw, isBinary = false) {
    ...
    if (message.type.startsWith('sensor.') && !validSensorMessage(message)) {
