...
class ZnpAdapterManager {
    ...
    async determineStrategy() {
        ...
        const configMatchesAdapter = nib &&
            ...
            (this.options.version !== tstype_1.ZnpVersion.ZStack12 || this.nwkOptions.networkKey.equals(preconfiguredKey.key)) &&
        ...
        if (configMatchesAdapter) {
            ...
            if (this.options.version !== tstype_1.ZnpVersion.ZStack12 && !this.nwkOptions.networkKey.equals(preconfiguredKey.key)) {
                logger_1.logger.warning("Adapter preconfigured (transport) key does not match configured network key " +
                    `(preconfigured=${preconfiguredKey.key.toString("hex")}, configured=${this.nwkOptions.networkKey.toString("hex")}). ` +
                    "This is typically harmless on Z-Stack 3.x adapters where only the active key matters.", NS);
            }
    ...
    async beginCommissioning(nwkOptions, failOnCollision = true, writeConfiguredFlag = true) {
        ...
        if ([tstype_1.ZnpVersion.ZStack30x, tstype_1.ZnpVersion.ZStack3x0].includes(this.options.version)) {
            ...
            const started = this.znp.waitFor(UnpiConstants.Type.AREQ, constants_1.Subsystem.ZDO, "stateChangeInd", undefined, undefined, 9, 100000);
