...
class Device extends entity_1.default {
    ...
    static isDeletedByIeeeAddr(ieeeAddr) {
        Device.loadFromDatabaseIfNecessary();
        return Device.deletedDevices.has(ieeeAddr);
    }
    /** Check if a device is explicitly deleted */
    static isDeletedByNetworkAddress(networkAddress) {
        Device.loadFromDatabaseIfNecessary();
        const ieeeAddr = Device.nwkToIeeeCache.get(networkAddress);
    ...
    interviewQuirks() {
        ...
        if (this._manufacturerID === 0xabcd) {
            ...
            logger_1.logger.debug("Interview - quirks matched for Control4 device", NS);
            return true;
        }
    ...
    async removeFromNetwork(clearCache = false) {
    ...
    removeFromDatabase(clearCache = false) {
        Device.loadFromDatabaseIfNecessary();
        for (const endpoint of this.endpoints) {
    ...
    #waitForOtaCommand(endpointId, commandId, defaultRspCommandId, timeout) {
        const waiter = entity_1.default.adapter.waitFor(this.networkAddress, endpointId, Zcl.FrameType.SPECIFIC, Zcl.Direction.CLIENT_TO_SERVER, undefined, GEN_OTA_CLUSTER_ID, commandId, defaultRspCommandId, timeout);
