...
const zclStatusError_1 = require("../zspec/zcl/zclStatusError");
...
class Controller extends node_events_1.default.EventEmitter {
    ...
    async start(abortSignal) {
        ...
        this.backupTimer = setInterval(() => this.backup().catch((err) => logger_1.logger.error(`Failed to backup: ${err}`, NS)), 86400000);
    ...
    async sendRaw(rawPayload, customClusters = {}) {
        ...
        if (interPan) {
            ...
            const zclFrame = Zcl.Frame.create(zcl.frameType ?? Zcl.FrameType.SPECIFIC, zcl.direction ?? Zcl.Direction.CLIENT_TO_SERVER, true, zcl.manufacturerCode, 0, zcl.commandKey, clusterKey ?? "touchlink", zcl.payload, customClusters);
            if (ieeeAddress) {
    ...
    async onZclPayload(payload) {
        ...
        if (payload.clusterID === Zcl.Clusters.greenPower.ID) {
            ...
            try {
                ...
                if (error instanceof zclStatusError_1.ZclStatusError) {
                    defaultResponse = error.code;
                }
            }
        }
        if (!device) {
            if (typeof payload.address === "number" && !model_1.Device.isDeletedByNetworkAddress(payload.address)) {
