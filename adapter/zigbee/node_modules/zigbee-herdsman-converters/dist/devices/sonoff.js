...
const SWVZNELegacyIrrigationAmountUnitByCode = {
    0: "us_gallon",
    1: "liter",
};
const SWVZNELegacyIrrigationAmountUnitCodeByName = {
    us_gallon: 0,
    liter: 1,
};
// Unified 0x5021 UnitOfWaterFlow mapping: 0 = liter, 1 = US gallon, 2 = imperial gallon
const SWVZNEUnifiedWaterFlowUnitByCode = {
    0: "liter",
    1: "us_gallon",
    2: "imperial_gallon",
};
const SWVZNEUnifiedWaterFlowUnitCodeByName = {
    liter: 0,
    us_gallon: 1,
    imperial_gallon: 2,
};
// Unified amount unit mapping used by 0x501D/0x501F/cmd 0x06/0x09: 0 = US gallon, 1 = liter, 2 = imperial gallon
const SWVZNEUnifiedIrrigationAmountUnitByCode = {
    0: "us_gallon",
    1: "liter",
    2: "imperial_gallon",
};
const SWVZNEUnifiedIrrigationAmountUnitCodeByName = {
    us_gallon: 0,
    liter: 1,
    imperial_gallon: 2,
};
const SWVZNELitersPerWaterFlowUnit = {
    liter: 1,
...
const SWVZNEFirmwareSupportsUnifiedImperialGallon = (device) => {
...
const SWVZNENormalizeWaterFlowUnit = (value) => {
    if (value === "gallon" || value === "US gallon" || value === "us_gallon")
        return "us_gallon";
    if (value === "liter" || value === "imperial_gallon")
        return value;
    return undefined;
};
const SWVZNEIrrigationAmountUnitFromDeviceCode = (code, device) => {
...
const SWVZNEIrrigationAmountUnitToDeviceCode = (unit, device) => {
    const normalizedUnit = SWVZNENormalizeWaterFlowUnit(unit);
    if (!normalizedUnit)
        return undefined;
    if (SWVZNEFirmwareSupportsUnifiedImperialGallon(device))
        return SWVZNEUnifiedIrrigationAmountUnitCodeByName[normalizedUnit];
    if (normalizedUnit === "imperial_gallon")
        return undefined;
...
const sonoffTrvzbtScheduleGroupLookup = { "1": 0, "2": 1, "3": 2 };
...
const sonoffTpWgzbaScheduleGroupLookup = { "1": 0, "2": 1, "3": 2 };
...
const sonoffTpWgzbaRelayOutputLookup = { normally_open_no: 0, normally_closed_nc: 1 };
...
const sonoffTpWgzbaTemperatureSensorSelectLookup = { internal: 0, external: 1, external_2: 2, external_3: 3 };
const sonoffTpWgzbaTemperatureSensorSelectValues = Object.keys(sonoffTpWgzbaTemperatureSensorSelectLookup);
const sonoffTpWgzbaDefaultTemperatureSensorSelect = "external";
const sonoffTpWgzbaTemporaryCommandModeLookup = { boost: 1, timer: 2 };
const sonoffTpWgzbaTemporaryAttributeModeLookup = { boost: 0, timer: 1 };
...
const formatSonoffTpWgzbaNtcTemperature = (value) => {
    utils.assertNumber(value, "currentNtcTemperature");
    const rawValue = value & 0xffff;
    const status = sonoffTpWgzbaNtcTemperatureStatusLookup[rawValue];
    if (status !== undefined)
        return status;
    const signedValue = rawValue >= 0x8000 ? rawValue - 0x10000 : rawValue;
    if (signedValue < sonoffTpWgzbaNtcTemperatureMinimumRaw)
        return sonoffTpWgzbaNtcTemperatureStatusLookup[32768];
    return `${(signedValue / sonoffTpWgzbaTemperatureScale).toFixed(2)} °C`;
};
const sonoffTrvzbtScheduleActiveNumCache = new Map();
const sonoffTrvzbtScheduleSelectedNumCache = new Map();
const sonoffTrvzbtTemperatureControlHistoryReqCache = {};
const sonoffTrvzbtTemperatureControlHistoryRespCache = {};
const sonoffTpWgzbaScheduleActiveNumCache = new Map();
const sonoffTpWgzbaScheduleSelectedNumCache = new Map();
const sonoffTpWgzbaTemperatureControlHistoryReqCache = {};
const sonoffTpWgzbaTemperatureControlHistoryRespCache = {};
const getValidSonoffTrvzbtScheduleGroupNum = (value) => {
    const scheduleGroup = Number(value);
    if (!Number.isInteger(scheduleGroup) ||
...
const getValidSonoffTrvzbtSchedulePublicGroup = (value) => {
...
const assertSonoffTrvzbtSchedulePublicGroup = (value, key) => {
...
const toSonoffTrvzbtSchedulePublicGroup = (scheduleGroup) => {
    return String(scheduleGroup + 1);
};
const toSonoffTrvzbtScheduleInternalGroup = (scheduleGroup) => {
...
const buildSonoffTrvzbtTemperatureControlHistoryResult = (request, state) => {
    ...
    return {
        ...
        heat_percentage_data: formatSonoffTrvzbtTemperatureControlHistoryOutputData(buildSonoffTrvzbtTemperatureControlHistoryData(request.type, valuesByDataType[0x01] ?? [], request)),
...
const formatSonoffTrvzbtFaultCode = (value) => {
    ...
    logger_1.logger.info(`TRV-ZBT formatSonoffTrvzbtFaultCode: rawValue=${rawValue} (0x${rawValue.toString(16).padStart(8, "0")}), faultBits=${faultBits} (0x${faultBits.toString(16).padStart(4, "0")}), matchedBits=${JSON.stringify(descriptions)}`, NS);
...
const formatMotorTravelCalibrationStatus = (value) => {
...
const cacheSonoffTrvzbtScheduleSelectedNum = (selectedNum, endpoint, device) => {
    const cacheKey = getSonoffTrvzbtDeviceCacheKey(endpoint, device);
    if (cacheKey) {
...
const getCachedSonoffTrvzbtScheduleSelectedNum = (endpoint, device) => {
    const cacheKey = getSonoffTrvzbtDeviceCacheKey(endpoint, device);
    return cacheKey ? sonoffTrvzbtScheduleSelectedNumCache.get(cacheKey) : undefined;
};
const getSonoffTrvzbtScheduleSelectedNum = (endpoint, device) => {
    return getCachedSonoffTrvzbtScheduleSelectedNum(endpoint, device) ?? getCachedSonoffTrvzbtScheduleActiveNum(endpoint, device) ?? 0;
};
const getSonoffTrvzbtEndpoint = (entity) => {
    return "read" in entity && typeof entity.read === "function" ? entity : undefined;
};
    ...
    if (endpoint) {
        try {
            ...
            if (activeNum !== undefined) {
                ...
                logger_1.logger.info(`TRV-ZBT ${reason}: active schedule group=${toSonoffTrvzbtSchedulePublicGroup(activeNum)}`, NS);
                return activeNum;
            }
...
const getSonoffTrvzbtScheduleNumFromMessage = (entity, device, message) => {
    const endpoint = getSonoffTrvzbtEndpoint(entity);
...
const shouldMirrorSonoffTrvzbtSelectedSchedule = (scheduleNum, meta, endpoint, device) => {
...
const buildSonoffTrvzbtSchedulePayload = (scheduleNum, dayofweek, transitions) => {
...
const sendSonoffTrvzbtScheduleReadCommand = async (entity, scheduleNum, reason) => {
    const payload = [0x01, 0x00, scheduleNum];
    logger_1.logger.info(`TRV-ZBT ${reason} scheduleGroup group=${toSonoffTrvzbtSchedulePublicGroup(scheduleNum)} payload=${formatSonoffTrvzbtPayload(payload)}`, NS);
    await entity.command("customSonoffTrvzbt", "scheduleGroup", { data: payload }, disableDefaultResponseOptions);
};
const getValidSonoffTpWgzbaScheduleGroupNum = (value) => {
    const scheduleGroup = Number(value);
    if (!Number.isInteger(scheduleGroup) ||
...
const getValidSonoffTpWgzbaSchedulePublicGroup = (value) => {
    const scheduleGroup = String(value);
    if (!(scheduleGroup in sonoffTpWgzbaScheduleGroupLookup))
        return;
    return scheduleGroup;
};
const assertSonoffTpWgzbaSchedulePublicGroup = (value, key) => {
...
const toSonoffTpWgzbaSchedulePublicGroup = (scheduleGroup) => `${scheduleGroup + 1}`;
const toSonoffTpWgzbaScheduleInternalGroup = (scheduleGroup) => {
    return sonoffTpWgzbaScheduleGroupLookup[scheduleGroup];
};
const getSonoffTpWgzbaTemperatureControlHistoryCacheKey = (endpoint, device) => {
    return getSonoffTrvzbtDeviceCacheKey(endpoint, device);
};
const getSonoffTpWgzbaTemperatureControlHistoryRespCacheKey = (cacheKey, subCmd) => {
    return `${cacheKey}:${subCmd}`;
};
const clearSonoffTpWgzbaTemperatureControlHistoryCache = (cacheKey, subCmd) => {
...
const buildSonoffTpWgzbaTemperatureControlHistoryData = (type, values, request) => {
    const records = [];
    let intervalStartSec = request.startUTC;
    for (const value of values) {
        const intervalEndSec = getSonoffTrvzbtTemperatureControlHistoryIntervalEnd(type, intervalStartSec, request.displayOffsetSeconds);
...
const buildSonoffTpWgzbaTemperatureControlHistoryResult = (request, state) => {
    const packets = Object.values(state.packets).sort((a, b) => a.current - b.current);
    const valuesByDataType = {};
    for (const historyPacket of packets) {
        valuesByDataType[historyPacket.dataType] ??= [];
    ...
    return {
        type: request.type,
        time_range: request.timeRange,
        temperature_data: formatSonoffTrvzbtTemperatureControlHistoryOutputData(buildSonoffTpWgzbaTemperatureControlHistoryData(request.type, valuesByDataType[0x00] ?? [], request)),
        heat_percentage_data: formatSonoffTrvzbtTemperatureControlHistoryOutputData(buildSonoffTpWgzbaTemperatureControlHistoryData(request.type, valuesByDataType[0x01] ?? [], request)),
        target_temperature_data: formatSonoffTrvzbtTemperatureControlHistoryOutputData(buildSonoffTpWgzbaTemperatureControlHistoryData(request.type, valuesByDataType[0x02] ?? [], request)),
    };
};
const cacheSonoffTpWgzbaScheduleActiveNum = (activeNum, endpoint, device) => {
    const cacheKey = getSonoffTrvzbtDeviceCacheKey(endpoint, device);
    if (cacheKey) {
        sonoffTpWgzbaScheduleActiveNumCache.set(cacheKey, activeNum);
    }
};
const cacheSonoffTpWgzbaScheduleSelectedNum = (selectedNum, endpoint, device) => {
    const cacheKey = getSonoffTrvzbtDeviceCacheKey(endpoint, device);
    if (cacheKey) {
        sonoffTpWgzbaScheduleSelectedNumCache.set(cacheKey, selectedNum);
    }
};
const getCachedSonoffTpWgzbaScheduleActiveNum = (endpoint, device) => {
    const cacheKey = getSonoffTrvzbtDeviceCacheKey(endpoint, device);
    return cacheKey ? sonoffTpWgzbaScheduleActiveNumCache.get(cacheKey) : undefined;
};
const getCachedSonoffTpWgzbaScheduleSelectedNum = (endpoint, device) => {
    const cacheKey = getSonoffTrvzbtDeviceCacheKey(endpoint, device);
    return cacheKey ? sonoffTpWgzbaScheduleSelectedNumCache.get(cacheKey) : undefined;
};
const getSonoffTpWgzbaScheduleSelectedNum = (endpoint, device) => {
...
const readSonoffTpWgzbaScheduleActiveNum = async (entity, device, reason) => {
    const endpoint = "read" in entity && typeof entity.read === "function" ? entity : undefined;
    if (endpoint) {
        try {
            const readResult = await endpoint.read("customSonoffTpWgzba", ["weeklyScheduleActiveNum"]);
            ...
            if (activeNum !== undefined) {
                ...
                logger_1.logger.info(`TP-WGZBA ${reason}: active schedule group=${toSonoffTpWgzbaSchedulePublicGroup(activeNum)}`, NS);
                return activeNum;
            }
            logger_1.logger.warning(`TP-WGZBA ${reason}: invalid active schedule group=${readResult.weeklyScheduleActiveNum}`, NS);
        }
        catch (error) {
            logger_1.logger.warning(`TP-WGZBA ${reason}: read active schedule group failed, ${error}`, NS);
        }
    }
    return getCachedSonoffTpWgzbaScheduleActiveNum(endpoint, device) ?? 0;
};
const getSonoffTpWgzbaScheduleNumFromMessage = (entity, device, message) => {
    const endpoint = "read" in entity && typeof entity.read === "function" ? entity : undefined;
    const selectedGroup = message?.schedule_group_to_edit;
    if (selectedGroup !== undefined) {
...
const shouldMirrorSonoffTpWgzbaSelectedSchedule = (scheduleNum, meta, endpoint, device) => {
    return scheduleNum === getSonoffTpWgzbaScheduleSelectedNum(endpoint, device ?? meta.device);
};
const getSonoffStructNumber = (value, index, elmType) => {
    if (!Array.isArray(value))
        return;
    const element = value[index];
    if (!element || (elmType !== undefined && element.elmType !== elmType))
        return;
    if (typeof element.elmVal !== "number")
        return;
    return element.elmVal;
};
const buildSonoffStruct = (elmType, values) => {
    return values.map((elmVal) => ({ elmType, elmVal }));
};
const parseSonoffTpWgzbaTimeValue = (value, key) => {
...
const formatSonoffTpWgzbaTimeValue = (value) => {
    const minutes = Math.max(0, Math.min(1439, value));
    const hours = Math.floor(minutes / 60);
    const mins = minutes % 60;
    return `${hours.toString().padStart(2, "0")}:${mins.toString().padStart(2, "0")}`;
};
const getSonoffTpWgzbaOutputBit = (value, key) => {
    if (value === undefined)
        return;
    if (typeof value === "string" && value in sonoffTpWgzbaRelayOutputLookup) {
        return sonoffTpWgzbaRelayOutputLookup[value];
    }
    throw new Error(`Invalid ${key}: expected ${Object.keys(sonoffTpWgzbaRelayOutputLookup).join(" or ")}`);
};
const isSonoffTpWgzbaExternalTemperatureInput = (value) => {
    return (utils.isNumber(value) &&
        Number.isFinite(value) &&
        value >= sonoffTpWgzbaExternalTemperatureInputRange.min &&
        value <= sonoffTpWgzbaExternalTemperatureInputRange.max);
};
const assertSonoffTpWgzbaExternalTemperatureInput = (value, key) => {
    if (!isSonoffTpWgzbaExternalTemperatureInput(value)) {
        throw new Error(`Invalid ${key}: expected number between ${sonoffTpWgzbaExternalTemperatureInputRange.min}-${sonoffTpWgzbaExternalTemperatureInputRange.max} °C (inclusive), got ${value}`);
    }
    return value;
};
const assertSonoffTpWgzbaTemperatureSensorSelect = (value, key) => {
    if (!utils.isString(value) || !sonoffTpWgzbaTemperatureSensorSelectValues.includes(value)) {
        throw new Error(`Invalid ${key}: expected ${sonoffTpWgzbaTemperatureSensorSelectValues.join(", ")}, got ${value}`);
    }
    return value;
};
const getSonoffTpWgzbaTemperatureSensorSelect = (status) => {
    for (const sensorSelect of sonoffTpWgzbaTemperatureSensorSelectValues) {
        if (sonoffTpWgzbaTemperatureSensorSelectLookup[sensorSelect] === status)
            return sensorSelect;
    }
};
const sonoffTpWgzbaExternalTemperatureInputToRaw = (value) => {
    return Math.round(value * sonoffTpWgzbaTemperatureScale);
};
const sonoffTpWgzbaRemoteTemperatureRawToExternalInput = (value) => {
    return utils.precisionRound(value / sonoffTpWgzbaTemperatureScale, sonoffTpWgzbaExternalTemperatureInputRange.precision);
};
const parseSonoffTpWgzbaRemoteAttributeLinkage = (value) => {
    const bytes = (0, sonoff_1.zclArrayValueToBytes)(value);
    if (bytes === undefined)
        return;
    const hasNoRemoteConfig = bytes.length >= 3 && bytes[0] === 0 && bytes.slice(3).every((byte) => byte === 0);
    if (bytes.length > 0 && (bytes.every((byte) => byte === 0) || hasNoRemoteConfig))
        return { temperature_sensor_select: "internal" };
    if (bytes.length < 3)
        return;
    const result = {};
...
const buildSonoffTpWgzbaRemoteAttributeLinkage = (temperatureSensorStatus, remoteTemperature) => {
...
const sendSonoffTpWgzbaScheduleReadCommand = async (entity, scheduleNum, reason) => {
    const payload = [0x01, 0x00, scheduleNum];
    logger_1.logger.info(`TP-WGZBA ${reason} scheduleGroup group=${toSonoffTpWgzbaSchedulePublicGroup(scheduleNum)} payload=${formatSonoffTrvzbtPayload(payload)}`, NS);
    await entity.command("customSonoffTpWgzba", "scheduleGroup", { data: payload }, disableDefaultResponseOptions);
};
const fzLocal = {
    key_action_event: {
...
const sonoffExtend = {
    ...
    trvzbtWeeklySchedule: () => {
        ...
        const scheduleDescription = 'The preset heating schedule to use when the system mode is set to "auto" (indicated with ⏲ on the TRV). ' +
            ...
            "(in 0.5°C steps). The temperature will be set at the time of the first transition until the time of the next transition, " +
            "e.g. '04:00/20 10:00/25' will result in the temperature being set to 20°C at 04:00 until 10:00, when it will change to 25°C.";
        const scheduleKeysFromMessage = (message) => {
            return message
                ? Object.keys(message).filter((key) => key.startsWith("weekly_schedule_") &&
                    sonoffTrvzbtScheduleDays.includes(key.replace("weekly_schedule_", "")))
                : [];
        };
        const exposes = [
            e
            ...
            ...sonoffTrvzbtScheduleDays.map((day) => e.text(`weekly_schedule_${day}`, ea.ALL).withCategory("config").withDescription(scheduleDescription)),
        ];
        const sendScheduleCommand = async (entity, scheduleNum, dayofweek, transitions) => {
            const payload = buildSonoffTrvzbtSchedulePayload(scheduleNum, dayofweek, transitions);
            logger_1.logger.info(`TRV-ZBT send scheduleGroup group=${toSonoffTrvzbtSchedulePublicGroup(scheduleNum)} ` +
                `dayofweek=0x${dayofweek.toString(16).padStart(2, "0")} transitions=${JSON.stringify(transitions)} ` +
                `payload=${formatSonoffTrvzbtPayload(payload)}`, NS);
            await entity.command(clusterName, scheduleGroupCommandName, { data: payload }, disableDefaultResponseOptions);
        };
        const fromZigbee = [
            {
                cluster: clusterName,
                type: ["attributeReport", "readResponse"],
                convert: (model, msg) => {
                    if (msg.data.weeklyScheduleActiveNum === undefined)
                        return;
                    const activeNum = getValidSonoffTrvzbtScheduleGroupNum(msg.data.weeklyScheduleActiveNum);
                    if (activeNum === undefined) {
                        logger_1.logger.warning(`TRV-ZBT received invalid weeklyScheduleActiveNum=${msg.data.weeklyScheduleActiveNum}`, NS);
                        return;
                    }
                    ...
                    logger_1.logger.info(`TRV-ZBT received weeklyScheduleActiveNum=${publicGroup}`, NS);
                    const result = { [activeGroupKey]: publicGroup };
            ...
            },
            {
                ...
                convert: (model, msg, publish, options, meta) => {
                    if (!(msg.data instanceof Buffer))
                        return;
                    const parsedRawCommand = (0, sonoff_1.parseSWVZFRawZclCommand)(msg.data);
                    if (parsedRawCommand?.commandId !== 0x13)
                        return;
                    const payload = parsedRawCommand.payload;
                    logger_1.logger.info(`TRV-ZBT received scheduleGroup payload=${formatSonoffTrvzbtPayload(payload)}`, NS);
                    if (payload.length < 4 || payload[0] !== 0x01)
                        return;
                    const readOrWrite = payload[1];
                    const scheduleNum = payload[2];
                    if (getValidSonoffTrvzbtScheduleGroupNum(scheduleNum) === undefined)
                        return;
                    if (readOrWrite === 0x01) {
                        const status = payload[3];
                        const statusText = status === 0x00 ? "success" : "fail";
                        logger_1.logger.info(`TRV-ZBT parsed scheduleGroup write response group=${toSonoffTrvzbtSchedulePublicGroup(scheduleNum)} ` +
                            `status=${statusText} rawStatus=0x${status.toString(16).padStart(2, "0")}`, NS);
                    ...
                    if (readOrWrite !== 0x00 || payload.length < 6)
                        return;
                    ...
                    logger_1.logger.info(`TRV-ZBT parsed scheduleGroup read response group=${toSonoffTrvzbtSchedulePublicGroup(scheduleNum)} ` +
                        `dayofweek=0x${dayofweek.toString(16).padStart(2, "0")} mode=0x${mode.toString(16).padStart(2, "0")} ` +
                        `transitions=${JSON.stringify(transitions)} schedule=${schedule}`, NS);
                    if (!shouldMirrorSonoffTrvzbtSelectedSchedule(scheduleNum, meta, msg.endpoint, msg.device))
                        return;
                    cacheSonoffTrvzbtScheduleSelectedNum(scheduleNum, msg.endpoint, msg.device);
                    const result = { [selectedGroupKey]: toSonoffTrvzbtSchedulePublicGroup(scheduleNum) };
        ...
        const toZigbee = [
            {
                ...
                convertSet: async (entity, key, value, meta) => {
                    ...
                    await entity.write(clusterName, { weeklyScheduleActiveNum: activeNum }, undefined);
                    cacheSonoffTrvzbtScheduleActiveNum(activeNum, getSonoffTrvzbtEndpoint(entity), meta.device);
                    return { state: { [key]: publicGroup } };
                },
                convertGet: async (entity) => {
                    await entity.read(clusterName, ["weeklyScheduleActiveNum"]);
                },
            },
            {
            ...
            },
            {
                ...
                convertSet: async (entity, key, value, meta) => {
                    ...
                    const stateUpdates = { [selectedGroupKey]: toSonoffTrvzbtSchedulePublicGroup(selectedNum) };
                    for (const [schedule, daysWithSchedule] of scheduleGroups) {
        ...
        const toZigbee = [
            {
                key: [key],
                convertGet: async (entity) => {
                    await entity.read(clusterName, ["faultCode"]);
                },
            },
        ];
        return { exposes, fromZigbee, toZigbee, isModernExtend: true };
        ...
        const fromZigbee = [
            {
                ...
                type: ["attributeReport", "readResponse"],
                convert: (model, msg) => {
                    if (msg.data.motorTravelCalibrationStatus === undefined)
                        return;
        ...
        const toZigbee = [
            {
                key: [key],
                convertGet: async (entity) => {
                    await entity.read(clusterName, ["motorTravelCalibrationStatus"]);
                },
            },
        ];
        return { exposes, fromZigbee, toZigbee, isModernExtend: true };
    },
    trvzbtHvacNotification: () => {
        const clusterName = "customSonoffTrvzbt";
        const exposes = [
        ...
        const fromZigbee = [
            {
                ...
                type: ["attributeReport", "readResponse"],
                convert: (model, msg) => {
                    if (msg.data.hvacMessageNotification === undefined)
                        return;
                    const data = Buffer.from(Array.from(msg.data.hvacMessageNotification));
                    if (data.length < 3 || data[0] !== 0x00 || data[1] < 1)
                        return;
        ...
        const fromZigbee = [
            {
                ...
                convert: (model, msg) => {
                    if (!(msg.data instanceof Buffer))
                        return;
                    const parsedRawCommand = (0, sonoff_1.parseSWVZFRawZclCommand)(msg.data);
                    if (parsedRawCommand?.commandId !== 0x10)
                        return;
                    const payload = parsedRawCommand.payload;
        ...
        const toZigbee = [
            {
                ...
                convertSet: async (entity, key, value) => {
                    ...
                    await entity.command(clusterName, commandName, { data: [0x03, 0x01, eventType] }, disableDefaultResponseOptions);
                    return { state: { [key]: value } };
                },
            },
        ];
    ...
    trvzbtTemporaryMode: () => {
        ...
        const exposes = [
            e
                ...
                .withValueStep(1)
                .withUnit("minutes")
                .withDescription("Boost Mode: Sets maximum TRV temperature for up to 180 minutes.Timer Mode: Customizes temperature and duration, up to 24 hours."))
                .withFeature(e
        ...
        const isValidTargetTemperature = (value) => {
        ...
        const fromZigbee = [
            {
                ...
                convert: (model, msg, publish, options, meta) => {
                    ...
                    const temporaryMode = utils.isObject(meta.state.temporary_mode) ? { ...meta.state.temporary_mode } : {};
        ...
        const toZigbee = [
            {
                ...
                convertSet: async (entity, key, value) => {
                    ...
                    await entity.write(clusterName, { temporaryModeTime }, undefined);
                    if (mode === "timer") {
                        utils.assertNumber(value.target_temperature, `${key}.target_temperature`);
                        validateRange(value.target_temperature, `${key}.target_temperature`, sonoffTrvzbtTargetTemperatureRange.min, sonoffTrvzbtTargetTemperatureRange.max);
                        const temporaryModeTemp = Math.round(value.target_temperature * sonoffTrvzbtTemporaryModeTemperatureScale);
                        await entity.write(clusterName, { temporaryModeTemp }, undefined);
                    }
                    await entity.write(clusterName, { temporaryMode }, undefined);
                    const state = { mode, duration: value.duration };
                    if (mode === "timer") {
                ...
                convertGet: async (entity) => {
                    await entity.read(clusterName, ["temporaryMode", "temporaryModeTime", "temporaryModeTemp"]);
                },
            },
        ];
        return { exposes, fromZigbee, toZigbee, isModernExtend: true };
    },
    trvzbtTemperatureControlHistory: () => {
        const clusterName = "customSonoffTrvzbt";
        const commandName = "readTemperatureControlHistory";
        const typeLookup = { day: 0x00, month: 0x01, half_year: 0x02 };
        const typeBySubCommand = { 0: "day", 1: "month", 2: "half_year" };
        const exposes = [
            e
        ...
        const fromZigbee = [
            {
                ...
                convert: (model, msg, publish, options, meta) => {
                    if (!(msg.data instanceof Buffer))
                        return;
                    const parsedRawCommand = (0, sonoff_1.parseSWVZFRawZclCommand)(msg.data);
                    if (parsedRawCommand?.commandId !== 0x0e)
                        return;
                    const payload = parsedRawCommand.payload;
                    logger_1.logger.info(`TRV-ZBT received temperature control history payload=${formatSonoffTrvzbtPayload(payload)}`, NS);
                    if (payload.length < 2)
                        return;
                    const subCmd = payload[0];
                    const type = typeBySubCommand[subCmd];
                    if (!type)
                        return;
                    ...
                    if (status !== 0x00) {
                        ...
                        logger_1.logger.error(`TRV-ZBT read temperature control history failed status=${status} subCmd=${subCmd}`, NS);
                        return;
                    }
                    if (payload.length < sonoffTrvzbtTemperatureControlHistoryValueOffset)
                        return;
                    if (!cacheKey || !respCacheKey) {
                        logger_1.logger.error(`TRV-ZBT missing temperature control history request context for subCmd=${subCmd}; it may have timed out, please try again`, NS);
                        return;
                    }
                    const now = Date.now();
                    ...
                    if (total === 0 || current >= total) {
                        logger_1.logger.error(`TRV-ZBT invalid temperature control history total=${total} current=${current} subCmd=${subCmd}`, NS);
                        return;
                    }
                    ...
                    clearSonoffTrvzbtTemperatureControlHistoryCache(cacheKey, subCmd);
                    logger_1.logger.info(`TRV-ZBT parsed temperature control history ${JSON.stringify(result)}`, NS);
        ...
        const toZigbee = [
            {
                ...
                convertSet: async (entity, key, value, meta) => {
                    ...
                    const startUtcSec = (0, sonoff_1.parseIsoWithOffsetToUtcSeconds)(timeStart);
                    const endUtcSec = (0, sonoff_1.parseIsoWithOffsetToUtcSeconds)(timeEnd);
                    if (startUtcSec === undefined || endUtcSec === undefined) {
                        throw new Error(`Invalid ${key}: expected ISO 8601 datetimes with timezone offset`);
                    }
                    const offsetSeconds = (0, sonoff_1.getRuntimeLocalOffsetSeconds)(Math.floor(Date.now() / 1000));
                    const startDeviceSec = (0, sonoff_1.utcToDeviceLocal2000Seconds)(startUtcSec, offsetSeconds);
                    const endDeviceSec = (0, sonoff_1.utcToDeviceLocal2000Seconds)(endUtcSec, offsetSeconds);
                    ...
                    const cacheKey = getSonoffTrvzbtTemperatureControlHistoryCacheKey(entity, meta?.device);
                    if (cacheKey) {
                    ...
                    logger_1.logger.info(`TRV-ZBT send temperature control history read type=${type} timeStart=${timeStart} timeEnd=${timeEnd} deviceStart=${startDeviceSec} deviceEnd=${endDeviceSec} payload=${formatSonoffTrvzbtPayload(payload)}`, NS);
                    await entity.command(clusterName, commandName, { data: Array.from(payload) }, disableDefaultResponseOptions);
    ...
    tpWgzbaTemperatureControlHistory: () => {
        ...
        const fromZigbee = [
            {
                ...
                convert: (model, msg, publish, options, meta) => {
                    ...
                    if (parsedRawCommand?.commandId !== 0x0e)
                        return;
                    const payload = parsedRawCommand.payload;
                    logger_1.logger.info(`TP-WGZBA received temperature control history payload=${formatSonoffTrvzbtPayload(payload)}`, NS);
                    if (payload.length < 2)
                        return;
                    const subCmd = payload[0];
                    const type = typeBySubCommand[subCmd];
                    ...
                    if (status !== 0x00) {
                        ...
                        logger_1.logger.error(`TP-WGZBA read temperature control history failed status=${status} subCmd=${subCmd}`, NS);
                        return;
                    }
                    if (payload.length < sonoffTpWgzbaTemperatureControlHistoryValueOffset)
                        return;
                    if (!cacheKey || !respCacheKey) {
                        logger_1.logger.error(`TP-WGZBA missing temperature control history request context for subCmd=${subCmd}; it may have timed out, please try again`, NS);
                        return;
                    }
                    const now = Date.now();
                    ...
                    const dataType = payload[6];
                    if (total === 0 || current >= total) {
                        logger_1.logger.error(`TP-WGZBA invalid temperature control history total=${total} current=${current} subCmd=${subCmd}`, NS);
                        return;
                    }
                    currentRequest.updatedAt = now;
                    const values = [];
                    ...
                    clearSonoffTpWgzbaTemperatureControlHistoryCache(cacheKey, subCmd);
                    logger_1.logger.info(`TP-WGZBA parsed temperature control history ${JSON.stringify(result)}`, NS);
                    return { temperature_control_history: result };
                },
            },
        ];
        const toZigbee = [
            {
                ...
                convertSet: async (entity, key, value, meta) => {
                    ...
                    payload.writeUInt32LE(endDeviceSec, 5);
                    payload.writeUInt16LE(0xffff, 9);
                    const cacheKey = getSonoffTpWgzbaTemperatureControlHistoryCacheKey(entity, meta?.device);
                    if (cacheKey) {
                    ...
                    logger_1.logger.info(`TP-WGZBA send temperature control history read type=${type} timeStart=${timeStart} timeEnd=${timeEnd} deviceStart=${startDeviceSec} deviceEnd=${endDeviceSec} payload=${formatSonoffTrvzbtPayload(payload)}`, NS);
                    await entity.command(clusterName, commandName, { data: Array.from(payload) }, disableDefaultResponseOptions);
                    return { state: { [key]: { type, time_range: { start: timeStart, end: timeEnd } } } };
                },
            },
    ...
    tpWgzbaWeeklySchedule: () => {
        const clusterName = "customSonoffTpWgzba";
        const commandName = "scheduleGroup";
        const activeGroupKey = "schedule_active_group";
        const selectedGroupKey = "schedule_group_to_edit";
        const scheduleDescription = 'The preset heating schedule to use when the system mode is set to "auto". ' +
        ...
        const sendScheduleCommand = async (entity, scheduleNum, dayofweek, transitions) => {
            const payload = buildSonoffTrvzbtSchedulePayload(scheduleNum, dayofweek, transitions);
            logger_1.logger.info(`TP-WGZBA send scheduleGroup group=${toSonoffTpWgzbaSchedulePublicGroup(scheduleNum)} dayofweek=0x${dayofweek.toString(16).padStart(2, "0")} transitions=${JSON.stringify(transitions)} payload=${formatSonoffTrvzbtPayload(payload)}`, NS);
            await entity.command(clusterName, commandName, { data: payload }, disableDefaultResponseOptions);
        };
        const fromZigbee = [
            {
                cluster: clusterName,
                ...
                convert: (model, msg) => {
                    ...
                    if (activeNum === undefined) {
                        logger_1.logger.warning(`TP-WGZBA received invalid weeklyScheduleActiveNum=${msg.data.weeklyScheduleActiveNum}`, NS);
                        return;
                    }
                    ...
                    logger_1.logger.info(`TP-WGZBA received weeklyScheduleActiveNum=${publicGroup}`, NS);
                    const result = { [activeGroupKey]: publicGroup };
            ...
            },
            {
                ...
                convert: (model, msg, publish, options, meta) => {
                    ...
                    if (parsedRawCommand?.commandId !== 0x13)
                        return;
                    const payload = parsedRawCommand.payload;
                    logger_1.logger.info(`TP-WGZBA received scheduleGroup payload=${formatSonoffTrvzbtPayload(payload)}`, NS);
                    if (payload.length < 4 || payload[0] !== 0x01)
                        return;
                    const readOrWrite = payload[1];
                    const scheduleNum = getValidSonoffTpWgzbaScheduleGroupNum(payload[2]);
                    if (scheduleNum === undefined)
                        return;
                    if (readOrWrite === 0x01) {
                        const status = payload[3];
                        logger_1.logger.info(`TP-WGZBA parsed scheduleGroup write response group=${toSonoffTpWgzbaSchedulePublicGroup(scheduleNum)} rawStatus=0x${status.toString(16).padStart(2, "0")}`, NS);
                        return;
                    }
                    if (readOrWrite !== 0x00 || payload.length < 6)
                        return;
                    ...
                    const schedule = formatSonoffTrvzbtScheduleTransitions(transitions);
                    logger_1.logger.info(`TP-WGZBA parsed scheduleGroup read response group=${toSonoffTpWgzbaSchedulePublicGroup(scheduleNum)} dayofweek=0x${dayofweek.toString(16).padStart(2, "0")} mode=0x${mode.toString(16).padStart(2, "0")} transitions=${JSON.stringify(transitions)} schedule=${schedule}`, NS);
                    if (!shouldMirrorSonoffTpWgzbaSelectedSchedule(scheduleNum, meta, msg.endpoint, msg.device))
                        return;
                    cacheSonoffTpWgzbaScheduleSelectedNum(scheduleNum, msg.endpoint, msg.device);
                    const result = { [selectedGroupKey]: toSonoffTpWgzbaSchedulePublicGroup(scheduleNum) };
                    for (const day of getSonoffTrvzbtScheduleDayNames(dayofweek)) {
                        result[`weekly_schedule_${day}`] = schedule;
                    }
                    return result;
        ...
        const toZigbee = [
            {
                ...
                convertSet: async (entity, key, value, meta) => {
                    ...
                    await entity.write(clusterName, { weeklyScheduleActiveNum: scheduleNum }, undefined);
                    return { state: { [key]: publicGroup } };
                },
                convertGet: async (entity) => {
                    await entity.read(clusterName, ["weeklyScheduleActiveNum"]);
            ...
            },
            {
                ...
                convertGet: async (entity, key, meta) => {
                    const endpoint = "read" in entity && typeof entity.read === "function" ? entity : undefined;
                    await sendSonoffTpWgzbaScheduleReadCommand(entity, getSonoffTpWgzbaScheduleSelectedNum(endpoint, meta.device), "read selected group");
                },
            },
            {
                key: sonoffTrvzbtScheduleDays.map((day) => `weekly_schedule_${day}`),
                convertSet: async (entity, key, value, meta) => {
                    ...
                    const stateUpdates = { [selectedGroupKey]: toSonoffTpWgzbaSchedulePublicGroup(scheduleNum) };
                    for (const [schedule, daysWithSchedule] of scheduleGroups) {
                        const parsed = parseSonoffTrvzbtScheduleString(schedule, daysWithSchedule.join(", "));
                        let dayofweek = 0;
                        for (const dayName of daysWithSchedule) {
    ...
    tpWgzbaReadScheduleOnConfigure: () => {
        const configure = [
            async (device) => {
                const endpoint = device.getEndpoint(1);
                if (!endpoint)
    ...
    tpWgzbaRemoteTemperatureSource: () => {
        const clusterName = "customSonoffTpWgzba";
        ...
        const exposes = [
            e
                ...
                .withDescription("Whether to use the value of the internal temperature sensor or an external temperature sensor for the perceived local temperature. Using an external sensor does not require local temperature calibration."),
            e
                ...
                .withUnit("°C")
                .withCategory("config")
                .withDescription("The value of an external temperature sensor. Note: synchronisation of this value with the external temperature sensor needs to happen outside of Zigbee2MQTT."),
        ];
        const writeRemoteAttributeLinkage = async (entity, temperatureSensorSelect, externalTemperatureInput) => {
            const payload = buildSonoffTpWgzbaRemoteAttributeLinkage(sonoffTpWgzbaTemperatureSensorSelectLookup[temperatureSensorSelect], sonoffTpWgzbaExternalTemperatureInputToRaw(externalTemperatureInput));
            await entity.write(clusterName, {
                [0x601e]: {
                    value: {
                        elementType: zigbee_herdsman_1.Zcl.DataType.UINT8,
                        elements: payload,
                    },
                    type: zigbee_herdsman_1.Zcl.DataType.ARRAY,
                },
            }, undefined);
        };
        const fromZigbee = [
            {
                ...
                type: ["attributeReport", "readResponse"],
                convert: (model, msg) => {
                    if (msg.data.remoteAttributeLinkage === undefined)
                        return;
        ...
        const toZigbee = [
            {
                ...
                convertSet: async (entity, key, value, meta) => {
                    ...
                    let temperatureSensorSelect = cachedTemperatureSensorSelect === undefined
                    ...
                    if (key === externalTemperatureInputKey) {
                        ...
                        if (externalTemperatureInput === undefined) {
                            if (temperatureSensorSelect !== "internal") {
                                throw new Error(`Invalid ${key}: external_temperature_input must be set before non-internal temperature_sensor_select`);
                            }
                ...
                convertGet: async (entity) => {
                    await entity.read(clusterName, ["remoteAttributeLinkage"]);
                },
            },
        ];
        return { exposes, fromZigbee, toZigbee, isModernExtend: true };
    },
    tpWgzbaTemperatureHysteresis: () => {
        const clusterName = "customSonoffTpWgzba";
        ...
        const fromZigbee = [
            {
                ...
                convert: (model, msg) => {
                    ...
                    if (value === undefined)
                        return;
                    const low = getSonoffStructNumber(value, 0, zigbee_herdsman_1.Zcl.DataType.INT16);
                    const high = getSonoffStructNumber(value, 1, zigbee_herdsman_1.Zcl.DataType.INT16);
        ...
        const toZigbee = [
            {
                ...
                convertSet: async (entity, key, value) => {
                    ...
                    await entity.write(clusterName, {
                        temperatureControlThreshold: buildSonoffStruct(zigbee_herdsman_1.Zcl.DataType.INT16, [
                ...
                convertGet: async (entity) => {
                    await entity.read(clusterName, ["temperatureControlThreshold"]);
                },
            },
        ];
        return { exposes, fromZigbee, toZigbee, isModernExtend: true };
    },
    tpWgzbaTimePeriod: (key, attribute, description) => {
        const clusterName = "customSonoffTpWgzba";
        const exposes = [
            e
                .composite(key, key, ea.ALL)
        ...
        const fromZigbee = [
            {
                ...
                convert: (model, msg) => {
                    ...
                    if (value === undefined)
                        return;
                    const start = getSonoffStructNumber(value, 0, zigbee_herdsman_1.Zcl.DataType.UINT16);
                    const end = getSonoffStructNumber(value, 1, zigbee_herdsman_1.Zcl.DataType.UINT16);
        ...
        const toZigbee = [
            {
                ...
                convertSet: async (entity, key, value) => {
                    ...
                    const period = buildSonoffStruct(zigbee_herdsman_1.Zcl.DataType.UINT16, [start, end]);
                    const payload = attribute === "radarDoNotDisturbPeriod" ? { radarDoNotDisturbPeriod: period } : { screenNightModePeriod: period };
                    await entity.write(clusterName, payload);
                    return { state: { [key]: { start: value.start, end: value.end } } };
                },
                convertGet: async (entity) => {
                    await entity.read(clusterName, [attribute]);
                },
            },
        ];
        return { exposes, fromZigbee, toZigbee, isModernExtend: true };
    ...
    tpWgzbaRelayOutput: () => {
        ...
        const relayOutputValueForBit = (bit) => {
        ...
        const exposes = [
            e
                ...
                .withCategory("config")
                .withDescription("Select how the 3 A relay operates. Normally open (NO): The output is powered during heating and unpowered when heating stops. Normally closed (NC): The output is unpowered during heating and powered when heating stops."),
            e
                .enum(boilerKey, ea.ALL, relayOutputKeys)
                .withCategory("config")
                .withDescription("Select how the boiler dry-contact relay operates. Normally open (NO): The contact closes during heating and opens when heating stops. Normally closed (NC): The contact opens during heating and closes when heating stops."),
        ];
        const fromZigbee = [
            {
                cluster: clusterName,
                type: ["attributeReport", "readResponse"],
                convert: (model, msg) => {
                    if (msg.data.relayOutputType === undefined)
                        return;
        ...
        const toZigbee = [
            {
                ...
                convertSet: async (entity, key, value, meta) => {
                    ...
                    await entity.write(clusterName, { relayOutputType }, undefined);
                    return { state: { [key]: value } };
                },
                convertGet: async (entity) => {
                    await entity.read(clusterName, ["relayOutputType"]);
                },
            },
        ];
        return { exposes, fromZigbee, toZigbee, isModernExtend: true };
    },
    tpWgzbaTemporaryMode: () => {
        const clusterName = "customSonoffTpWgzba";
        ...
        const exposes = [
            e
                ...
                .withDescription("Boost Mode: Runs the heating at the maximum set temperature for a user-defined duration to quickly warm the room.Timer Mode: Runs the heating at a user-defined temperature for a specified duration. When the timer ends, the thermostat returns to its previous mode and set temperature."))
                .withFeature(e
                .numeric("duration", ea.ALL)
                .withValueMin(0)
                .withValueMax(1440)
                .withValueStep(5)
                .withUnit("minutes")
                .withDescription("Boost Mode: Runs the heating at the maximum set temperature for up to 180 minutes.Timer Mode: Runs the heating at a custom temperature for a specified duration of up to 24 hours."))
                .withFeature(e
                .numeric("target_temperature", ea.ALL)
            ...
            },
            {
                ...
                convert: (model, msg, publish, options, meta) => {
                    if (!(msg.data instanceof Buffer))
                        return;
                    const parsedRawCommand = (0, sonoff_1.parseSWVZFRawZclCommand)(msg.data);
                    if (parsedRawCommand?.commandId !== 0x11)
                        return;
                    const payload = parsedRawCommand.payload;
                    if (payload.length < 6)
                        return;
                    const status = payload[0];
                    const statusText = sonoffTpWgzbaTemporaryModeStatusLookup[status] ?? "unknown";
                    logger_1.logger.info(`TP-WGZBA temporary mode response status=${statusText} payload=${formatSonoffTrvzbtPayload(payload)}`, NS);
                    if (status !== 0x00)
                        return;
                    const temporaryMode = utils.isObject(meta.state.temporary_mode) ? { ...meta.state.temporary_mode } : {};
        ...
        const toZigbee = [
            {
                ...
                convertSet: async (entity, key, value) => {
                    ...
                    if (mode === "timer") {
                        ...
                        if (!isValidTargetTemperature(value.target_temperature)) {
                            throw new Error(`Invalid ${key}.target_temperature: expected value between ${sonoffTpWgzbaTargetTemperatureRange.min}-${sonoffTpWgzbaTargetTemperatureRange.max} (inclusive), got ${value.target_temperature}`);
                        }
                ...
                convertGet: async (entity) => {
                    await entity.command(clusterName, commandName, { data: [0xff, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00] }, disableDefaultResponseOptions);
                },
            },
        ];
        return { exposes, fromZigbee, toZigbee, isModernExtend: true };
    },
    tpWgzbaHvacNotification: () => {
        const clusterName = "customSonoffTpWgzba";
        const exposes = [
            e.binary("open_window_detected", ea.STATE, true, false).withDescription("Indicates whether open window detection was triggered."),
        ];
        ...
        const fromZigbee = [
            {
                ...
                type: ["attributeReport", "readResponse"],
                convert: (model, msg) => {
                    if (msg.data.currentNtcTemperature === undefined)
                        return;
    ...
    buildOverloadProtectionPayload: (value, powerMaxLimit, currentMaxLimit) => {
        ...
        const payloadValue = new Uint8Array(30);
        let index = 0;
            ...
            (event) => {
                if (event.type === "start" && !event.data.device.customReadResponse) {
                    event.data.device.customReadResponse = (frame, endpoint) => {
                        ...
                        const { time, timeZone, localTime, dstShift } = (0, zigbee_herdsman_1.getTimeClusterAttributes)();
                        ...
                        endpoint
                            ...
                            .catch((e) => {
                            logger_1.logger.warning(`SWV custom time response failed: ${e}`, NS);
                        });
                        return true;
                    };
                }
        ...
        const fromZigbee = [
            {
                ...
                type: ["attributeReport", "readResponse"],
                convert: (model, msg, publish, options, meta) => {
                    if (msg.data.rainDelayEndDatetime === undefined || msg.data.rainDelayEndDatetime === null)
                        return;
                    ...
                    const offsetSeconds = (0, sonoff_1.getRuntimeLocalOffsetSeconds)(value + sonoff_1.YEAR_2000_IN_UTC);
                    const utcSeconds = (0, sonoff_1.deviceLocal2000ToUTCSeconds)(value, offsetSeconds);
                    return {
                        rain_delay_end_datetime: (0, sonoff_1.formatUtcSecondsToIsoWithOffset)(utcSeconds, offsetSeconds),
                    };
                },
            },
        ];
    ...
    valveAbnormalState: (dualChannel = false) => {
        const maxMask = dualChannel ? 0b111111 : 0b1111;
        const getValveAbnormalStates = (value) => {
            const states = [];
            if ((value & 0b000001) !== 0)
                states.push(dualChannel ? "water_shortage_channel_1" : "water_shortage");
            if ((value & 0b000010) !== 0)
                states.push("water_leakage");
            if ((value & 0b001000) !== 0)
                states.push(dualChannel ? "fail_safe_channel_1" : "fail_safe");
            if (dualChannel && (value & 0b010000) !== 0)
                states.push("water_shortage_channel_2");
            if (dualChannel && (value & 0b100000) !== 0)
                states.push("fail_safe_channel_2");
            return states;
        };
        // Generate all combinations from the defined state bits
        const allCombinations = ["normal"];
        ...
        const fromZigbee = [
            {
                ...
                type: ["attributeReport", "readResponse"],
                convert: (model, msg, publish, options, meta) => {
                    if (msg.data.valveAbnormalState === undefined)
                        return;
    ...
    manualDefaultSettings: (hasFlowMeter) => {
        const exposes = (device) => {
            const expose = e
                ...
                .withFeature(e.numeric("irrigation_duration", ea.ALL).withValueMin(1).withValueMax(719).withUnit("min").withDescription("Irrigation duration"));
            if (hasFlowMeter) {
                expose.withFeature(e.enum("irrigation_mode", ea.ALL, ["duration", "capacity"]).withDescription("Irrigation mode: duration or capacity"));
                if (!(utils.isDummyDevice(device) || SWVZNEFirmwareSupportsUnifiedImperialGallon(device))) {
                ...
                expose
                    ...
                    .withFeature(e.numeric("fail_safe", ea.ALL).withValueMin(0).withValueMax(719).withUnit("min").withDescription("Safety protection timeout"));
            }
            return [expose];
        };
        const modeMap = {
        ...
        const fromZigbee = [
            {
                ...
                type: ["attributeReport", "readResponse"],
                convert: (model, msg, publish, options, meta) => {
                    if (!msg.data.manualDefaultSettings)
                        return;
                    const array = new Uint8Array(msg.data.manualDefaultSettings);
                    if (array.length < 12) {
                        logger_1.logger.error(`manualDefaultSettings invalid length=${array.length}, expected>=12`, NS);
                        return;
                    }
                    const mode = array[0];
        ...
        const toZigbee = [
            {
                ...
                convertSet: async (entity, key, value, meta) => {
                    ...
                    if (hasFlowMeter && (typeof value.irrigation_mode !== "string" || modeMap[value.irrigation_mode] === undefined)) {
                        logger_1.logger.error("manual_default_settings invalid irrigation_mode, expected one of: duration, capacity.", NS);
                        return;
                    }
                    ...
                    if (hasFlowMeter) {
                        ...
                        if (parsedCapacityUnit === undefined) {
                            logger_1.logger.error("manual_default_settings invalid irrigation_amount_unit, expected one of: us_gallon, liter.", NS);
                            return;
                        }
                        capacityUnit = parsedCapacityUnit;
                    }
                    const parseRequiredInt = (fieldName) => {
                        const parsed = Number(value[fieldName]);
                        if (!Number.isInteger(parsed)) {
                            logger_1.logger.error(`manual_default_settings invalid ${fieldName}, expected integer.`, NS);
                            return;
                        }
                        return parsed;
                    };
                ...
                convertGet: async (entity, key, meta) => {
                    await entity.read("customClusterEwelink", ["manualDefaultSettings"]);
                },
            },
        ];
        return {
    ...
    seasonalWateringAdjustment: () => {
        ...
        for (const month of months) {
            exposesComposite = exposesComposite.withFeature(e.numeric(month, ea.ALL).withValueMin(0.1).withValueMax(2).withValueStep(0.1).withDescription(`Adjustment multiplier for ${month}`));
        }
        const fromZigbee = [
            {
                cluster: "customClusterEwelink",
                type: ["attributeReport", "readResponse"],
                convert: (model, msg, publish, options, meta) => {
                    if (!msg.data.seasonalWateringAdjustment)
                        return;
                    const array = new Uint8Array(msg.data.seasonalWateringAdjustment);
                    if (array.length < 12) {
                        logger_1.logger.error(`seasonalWateringAdjustment invalid length=${array.length}, expected>=12`, NS);
                        return;
                    }
                    const result = {};
                    for (let i = 0; i < 12; i++) {
        ...
        const toZigbee = [
            {
                ...
                convertGet: async (entity, key, meta) => {
                    await entity.read("customClusterEwelink", ["seasonalWateringAdjustment"]);
                },
            },
        ];
        return {
    ...
    irrigationScheduleStatus: (hasFlowMeter, endpointNames) => {
        const expose = (device) => {
            const baseExpose = e
                ...
                .withFeature(e
                .enum("irrigation_mode", ea.STATE, hasFlowMeter ? ["duration", "capacity", "duration_with_interval"] : ["duration", "duration_with_interval"])
            ...
            if (hasFlowMeter) {
                const unitValues = utils.isDummyDevice(device) || SWVZNEFirmwareSupportsUnifiedImperialGallon(device)
        ...
        const scheduleStatusMapReverse = {
            [0x00]: "start",
            [0x01]: "end",
            [0x02]: "running",
            [0x03]: "standby",
        ...
        const fromZigbee = [
            {
                ...
                type: ["attributeReport", "readResponse"],
                convert: (model, msg, publish, options, meta) => {
                    if (!msg.data.irrigationScheduleStatus)
                        return;
                    if (endpointNames) {
                        const endpointName = utils.getKey(model.endpoint?.(meta.device) ?? {}, msg.endpoint.ID);
                        if (!endpointName || !endpointNames.includes(endpointName))
                            return;
                    }
                    ...
                    const offsetSeconds = (0, sonoff_1.getRuntimeLocalOffsetSeconds)(Math.floor(Date.now() / 1000));
                    const toIsoString = (deviceSeconds) => {
                        const utcSeconds = (0, sonoff_1.deviceLocal2000ToUTCSeconds)(deviceSeconds, offsetSeconds);
                        return (0, sonoff_1.formatUtcSecondsToIsoWithOffset)(utcSeconds, offsetSeconds);
                    };
                    // Start or Standby: 15 bytes
                    if (isStartOrStandby) {
                        const minLength = 15;
                        if (array.length < minLength) {
                            logger_1.logger.error(`irrigationScheduleStatus invalid length=${array.length}, expected>=${minLength} for start/standby`, NS);
                            return;
                        }
                    ...
                    if (scheduleStatus === scheduleStatusMap.end || scheduleStatus === scheduleStatusMap.running) {
                        const minLength = 21;
                        if (array.length < minLength) {
                            logger_1.logger.error(`irrigationScheduleStatus invalid length=${array.length}, expected>=${minLength} for end/running`, NS);
                            return;
                        }
                        const expectedStartTime = (array[4] << 24) | (array[5] << 16) | (array[6] << 8) | array[7];
                        const expectedEndTime = (array[8] << 24) | (array[9] << 16) | (array[10] << 8) | array[11];
        ...
        const toZigbee = [
            {
                ...
                convertGet: async (entity, key, meta) => {
                    await utils
                        ...
                        .read("customClusterEwelink", ["irrigationScheduleStatus"]);
                },
            },
        ];
        return {
        ...
        const fromZigbee = [
            {
                ...
                type: ["attributeReport", "readResponse"],
                convert: (model, msg, publish, options, meta) => {
                    if (!msg.data.valveAlarmSettings)
                        return;
                    const array = new Uint8Array(msg.data.valveAlarmSettings);
                    if (array.length < 4) {
                        logger_1.logger.error(`valveAlarmSettings invalid length=${array.length}, expected>=4`, NS);
                        return;
                    }
        ...
        const toZigbee = [
            {
                ...
                convertSet: async (entity, key, value, meta) => {
                    ...
                    let enableBits = 0;
                    if (state.enable_alarm_water_shortage)
                        enableBits |= 0b00001;
                    if (state.enable_alarm_water_leak)
                        enableBits |= 0b00010;
                    if (state.enable_water_shortage_auto_close)
                        enableBits |= 0b01000;
                ...
                convertGet: async (entity, key, meta) => {
                    await entity.read("customClusterEwelink", ["valveAlarmSettings"]);
                },
            },
        ];
        return {
    ...
    waterFlowUnit: () => {
        const expose = (device) => utils.isDummyDevice(device) || SWVZNEFirmwareSupportsUnifiedImperialGallon(device)
            ? [
                e
        ...
        const fromZigbee = [
            {
                ...
                type: ["attributeReport", "readResponse"],
                convert: (model, msg, publish, options, meta) => {
                    if (msg.data.unitOfWaterFlow === undefined)
                        return;
                    ...
                    if (utils.isObject(settings) && typeof settings.irrigation_amount === "number") {
                        ...
                        if (sourceUnit && sourceUnit !== waterFlowUnit) {
                            const irrigationAmountLiters = typeof settings.irrigation_amount_real_liter === "number"
        ...
        const toZigbee = [
            {
                ...
                convertSet: async (entity, key, value, meta) => {
                    const device = meta.device;
                    if (!device) {
                        logger_1.logger.error("water_flow_unit cannot be set: meta.device is undefined", NS);
                        return;
                    }
                    if (!SWVZNEFirmwareSupportsUnifiedImperialGallon(device)) {
                        const minVersion = device.modelID === "SWV-ZF2" ? "1.0.9" : "1.1.0";
                        logger_1.logger.warning(`water_flow_unit is only supported by SWV firmware >= ${minVersion}, skip writing unitOfWaterFlow.`, NS);
                        return;
                    }
                    const normalizedUnit = SWVZNENormalizeWaterFlowUnit(value);
                    if (!normalizedUnit) {
                        logger_1.logger.error("water_flow_unit invalid value, expected one of: liter, us_gallon, imperial_gallon.", NS);
                        return;
                    }
                    const waterFlowUnitCode = SWVZNEUnifiedWaterFlowUnitCodeByName[normalizedUnit];
                    await entity.write("customClusterEwelink", { unitOfWaterFlow: waterFlowUnitCode }, utils.getOptions(meta.mapped, entity));
                    const settings = meta.state.manual_default_settings;
                    let manualDefaultSettings;
                    if (utils.isObject(settings) && typeof settings.irrigation_amount === "number") {
                        const sourceUnit = SWVZNENormalizeWaterFlowUnit(settings.irrigation_amount_unit ?? meta.state.water_flow_unit);
                ...
                convertGet: async (entity, key, meta) => {
                    if (!SWVZNEFirmwareSupportsUnifiedImperialGallon(meta.device))
                        return;
                    await entity.read("customClusterEwelink", ["unitOfWaterFlow"]);
                },
            },
        ];
        return {
    ...
    readSWVZFRecord: (hasFlowMeter, dualChannel = false) => {
        ...
        const normalizeUtcEpochSeconds = (input, label) => {
            if (typeof input !== "string") {
                logger_1.logger.error(`read_swvzf_records invalid ${label}: expected ISO 8601 string.`, NS);
                return;
            }
            const seconds = (0, sonoff_1.parseIsoWithOffsetToUtcSeconds)(input);
            if (seconds === undefined) {
                logger_1.logger.error(`read_swvzf_records invalid ${label}: expected ISO 8601 datetime with timezone offset (Z or ±HH:mm).`, NS);
                return;
            }
            return seconds;
        };
        const parseHistoryRecords = (mode, recordData, args) => {
            const records = [];
            ...
            const createHistoryRecord = (duration, start, end, volume, channel) => {
            ...
            if (mode === "24_hours") {
                ...
                for (let i = 0; i < count; i++) {
                    ...
                    const start = (0, sonoff_1.formatUtcSecondsToIsoWithOffset)(Math.floor(startMs / 1000), args.offsetSeconds);
                    const end = (0, sonoff_1.formatUtcSecondsToIsoWithOffset)(Math.floor(endMs / 1000), args.offsetSeconds);
                    if (channelCount === 1) {
                        const index = i * 3;
            ...
            if (mode === "30_days") {
                ...
                for (let i = 0; i < count; i++) {
                    ...
                    if (channelCount === 1) {
                        ...
                        const channel1Volume = (recordData[channel1Index + 2] << 16) + (recordData[channel1Index + 3] << 8) + recordData[channel1Index + 4];
                        records.push(createHistoryRecord(channel1Duration, start, end, channel1Volume, "1"));
                        const channel2Index = channel1Index + recordByteLength;
                        const channel2Duration = (recordData[channel2Index] << 8) + recordData[channel2Index + 1];
                        const channel2Volume = (recordData[channel2Index + 2] << 16) + (recordData[channel2Index + 3] << 8) + recordData[channel2Index + 4];
                        records.push(createHistoryRecord(channel2Duration, start, end, channel2Volume, "2"));
                    }
                    startMs = endMs;
                }
            ...
            if (mode === "6_months") {
                ...
                for (let i = 0; i < count; i++) {
                    const intervalEndSec = (0, sonoff_1.shiftUtcSecondsByOffsetMonths)(intervalStartSec, 1, args.offsetSeconds);
                    const start = (0, sonoff_1.formatUtcSecondsToIsoWithOffset)(intervalStartSec, args.offsetSeconds);
                    const end = (0, sonoff_1.formatUtcSecondsToIsoWithOffset)(intervalEndSec, args.offsetSeconds);
                    if (channelCount === 1) {
                        const index = i * 5;
                        const duration = (recordData[index] << 8) + recordData[index + 1];
                        const volume = (recordData[index + 2] << 16) + (recordData[index + 3] << 8) + recordData[index + 4];
        ...
        const cacheMultiPacketRecords = (ieeeAddr, recordIndex, records, expectedPacketCount, stateKey, subCmd, now) => {
        ...
        const toZigbee = [
            {
                ...
                convertSet: async (entity, key, value, meta) => {
                    ...
                    if (!["24_hours", "30_days", "6_months"].includes(type)) {
                        logger_1.logger.error(`read_swvzf_records invalid type: ${type}`, NS);
                        return;
                    }
                    if (value?.time_start === undefined || value?.time_end === undefined) {
                        logger_1.logger.error("read_swvzf_records requires time_start and time_end", NS);
                        return;
                    }
                    ...
                    const endDeviceSec = (0, sonoff_1.utcToDeviceLocal2000Seconds)(endUtcSec, offsetSeconds);
                    if (startDeviceSec < 0 || startDeviceSec > 0xffffffff || endDeviceSec < 0 || endDeviceSec > 0xffffffff) {
                        logger_1.logger.error("read_swvzf_records time range out of supported 2000-local uint32 range", NS);
                        return;
                    }
                    if (endDeviceSec < startDeviceSec) {
                        logger_1.logger.error("read_swvzf_records time_end earlier than time_start", NS);
                        return;
                    }
                    let subCmd = 0x00;
                    if (type === "24_hours")
                    ...
                    await entity.command(clusterName, commandName, { data: payloadValue }, { disableDefaultResponse: true });
                },
            },
        ];
        const fromZigbee = [
            {
                ...
                convert: (model, msg, publish, options, meta) => {
                    ...
                    if (!parsedRawCommand)
                        return;
                    if (parsedRawCommand.commandId !== 0x00)
                        return;
                    ...
                    if (status !== 0x00) {
                        ...
                        let errorReason = "";
                        if (status === 0x01)
                            errorReason = "timestamp_invalid";
                        else if (status === 0x02)
                            errorReason = "watering_amount_invalid";
                        else if (status === 0x03)
                            errorReason = "irrigation_duration_invalid";
                        logger_1.logger.error(`readSWVZFRecord invalid status=${status}===${errorReason} subCmd=${subCmd} recordIndex=${recordIndex}`, NS);
                        return;
                    }
                    const now = Date.now();
                    ...
                    if (!request || typeof request.startUTC !== "number" || typeof request.endUTC !== "number") {
                        logger_1.logger.error(`readSWVZFRecord missing request context for subCmd=${subCmd}; it may have timed out, please try again`, NS);
                        return;
                    }
                    ...
                    if (subCmd === 0) {
                        ...
                        if (recordIndex >= expectedPacketCount)
                            return;
                        ...
                        if (recordData.length < intervalByteLength)
                            return;
                        ...
                        if (channelCount === 1) {
                            ...
                            if (recordIndex === 0 || recordIndex === 1) {
                                if (payload.length % 5 !== 0) {
                                    logger_1.logger.info(`readSWVZFRecord invalid 30_days payloadLen=${payload.length} recordIndex=${recordIndex}`, NS);
                                    return;
                                }
                                parsedDayCount = Math.floor(payload.length / 5);
                                if (parsedDayCount !== 10) {
                                    logger_1.logger.info(`readSWVZFRecord invalid fixed dayCount=${parsedDayCount} recordIndex=${recordIndex}`, NS);
                                    return;
                                }
                                ...
                                if ((payload.length - 1) % 5 !== 0) {
                                    logger_1.logger.info(`readSWVZFRecord invalid 30_days payloadLen=${payload.length} recordIndex=2`, NS);
                                    return;
                                }
                                parsedDayCount = dayCount;
                                if (parsedDayCount < 8 || parsedDayCount > 11) {
                                    logger_1.logger.info(`readSWVZFRecord invalid dayCount=${parsedDayCount} recordIndex=2`, NS);
                                    return;
                                }
                                const actualCount = Math.floor((payload.length - 1) / 5);
                                if (actualCount !== parsedDayCount) {
                                    logger_1.logger.info(`readSWVZFRecord dayCount mismatch dayCount=${parsedDayCount} actualCount=${actualCount} recordIndex=2`, NS);
                                    return;
                                }
                        ...
                        if (channelCount === 2) {
                            ...
                            if (recordIndex < 4) {
                                ...
                                if (parsedDayCount < 0 || parsedDayCount > 3) {
                                    logger_1.logger.info(`readSWVZFRecord invalid dayCount=${parsedDayCount} recordIndex=4`, NS);
                                    return;
                                }
                                const expectedLength = 1 + parsedDayCount * intervalByteLength;
                                if (payload.length < expectedLength) {
                                    logger_1.logger.info(`readSWVZFRecord invalid 30_days payloadLen=${payload.length} recordIndex=4`, NS);
                                    return;
                                }
                                recordData = Array.from(payload.subarray(1, expectedLength));
                            }
                            const dayOffset = recordIndex * 7;
                            const records = parsedDayCount === 0
                                ? []
    ...
    irrigationPlanSettingsAndReport: (hasFlowMeter, endpointNames) => {
        const clusterName = "customClusterEwelink";
        const commandId = {
            irrigationPlanSettings: 0x06,
            irrigationPlanReport: 0x09,
        };
        const loopTypeModeMapping = {
            0: "odd_days",
            1: "even_days",
            2: "day_interval",
            3: "weekdays",
        ...
        const loopTypeWeekDayNames = Object.keys(loopTypeWeekDayBitMapping);
        const decodeLoopTypeWeekDays = (mask) => ({
        ...
        const expose = (device) => {
            const irrigationPlanSettings = e
                ...
                .withFeature(e.enum("irrigation_mode", ea.SET, hasFlowMeter ? ["duration", "capacity", "duration_with_interval"] : ["duration", "duration_with_interval"]))
            ...
            irrigationPlanSettings.withFeature(e.text("create_datetime", ea.SET).withDescription("Create datetime in ISO format with timezone (e.g. YYYY-MM-DDTHH:mm:ss+08:00)"));
            const irrigationPlanReport = e
                ...
                .withFeature(e.enum("irrigation_mode", ea.STATE, hasFlowMeter ? ["duration", "capacity", "duration_with_interval"] : ["duration", "duration_with_interval"]))
            ...
            irrigationPlanReport.withFeature(e.text("create_datetime", ea.STATE).withDescription("Create datetime in ISO format with timezone (e.g. YYYY-MM-DDTHH:mm:ss+08:00)"));
        ...
        const fromZigbee = [
            {
                ...
                convert: (model, msg, publish, options, meta) => {
                    ...
                    if (cmdId === commandId.irrigationPlanSettings) {
                        if (payload.length < 1) {
                            logger_1.logger.error("irrigationPlanSettingsReply invalid payload length", NS);
                            return;
                        }
                        const status = payload.readUInt8(0);
                        if (status !== 0) {
                            logger_1.logger.error(`irrigationPlanSettingsReply failed, status=${status}`, NS);
                        }
                        return;
                    }
                    if (cmdId === commandId.irrigationPlanReport) {
                        ...
                        const offsetSeconds = (0, sonoff_1.getRuntimeLocalOffsetSeconds)(enableDatetimeDevice + sonoff_1.YEAR_2000_IN_UTC);
                        const enableDatetimeUTC = (0, sonoff_1.deviceLocal2000ToUTCSeconds)(enableDatetimeDevice, offsetSeconds);
                        offset += 4;
                        // Irrigation mode
                        ...
                        const enableDateISO = (0, sonoff_1.formatUtcSecondsToIsoWithOffset)(enableDatetimeUTC, offsetSeconds);
                        ...
                        const createDatetimeISO = (0, sonoff_1.formatUtcSecondsToIsoWithOffset)(createDatetimeDevice, offsetSeconds);
                        const plan = {
                            ...
                            loop_type_week_days: loopTypeMode === loopTypeModeMappingReverse.weekdays
        ...
        const toZigbee = [
            {
                ...
                convertSet: async (entity, key, value, meta) => {
                    utils.assertObject(value, key);
                    const parseIntWithDefault = (fieldName, defaultValue, min, max) => {
                        const raw = value[fieldName];
                        const parsed = raw === undefined || raw === null ? defaultValue : Number(raw);
                        if (!Number.isInteger(parsed) || parsed < min || parsed > max) {
                            logger_1.logger.error(`irrigation_plan_settings invalid ${fieldName}, expected integer in range [${min}, ${max}].`, NS);
                            return;
                        }
                        return parsed;
                    };
                    const payloadValue = new Uint8Array(28);
                    let i = 0;
                    // Plan index
                    ...
                    // Loop type
                    const loopTypeModeKey = typeof value.loop_type_mode === "string" && value.loop_type_mode in loopTypeModeMappingReverse
                        ? value.loop_type_mode
                        : "odd_days";
                    ...
                    if (!utils.isString(enableDateValue)) {
                        logger_1.logger.error("irrigation_plan_settings invalid enable_date, expected local date in YYYY-MM-DD format.", NS);
                        return;
                    }
                    ...
                    if (enableDateUtc.getUTCFullYear() !== enableYear ||
                        enableDateUtc.getUTCMonth() !== enableMonth - 1 ||
                        enableDateUtc.getUTCDate() !== enableDay) {
                        logger_1.logger.error("irrigation_plan_settings invalid enable_date, expected local date in YYYY-MM-DD format.", NS);
                        return;
                    }
                    const enableDatetime = Math.floor(enableDateUtcMs / 1000) - sonoff_1.YEAR_2000_IN_UTC;
                    if (enableDatetime < 0 || enableDatetime > 0xffffffff) {
                        logger_1.logger.error("irrigation_plan_settings invalid enable_date, converted 2000-local value out of uint32 range.", NS);
                        return;
                    }
                    ...
                    if (hasFlowMeter) {
                        const irrigationModeKey = typeof value.irrigation_mode === "string"
                            ? value.irrigation_mode
                            : "duration";
                    ...
                    if (!utils.isString(startTimeValue)) {
                        logger_1.logger.error("irrigation_plan_settings invalid start_time, expected HH:mm.", NS);
                        return;
                    }
                    ...
                    if (irrigationAmountUnitCode === undefined) {
                        logger_1.logger.error("irrigation_plan_settings invalid irrigation_amount_unit, expected one of: us_gallon, liter.", NS);
                        return;
                    }
                    ...
                    if (!utils.isString(value.create_datetime)) {
                        logger_1.logger.error("irrigation_plan_settings invalid create_datetime, expected ISO 8601 datetime with timezone offset (Z or ±HH:mm).", NS);
                        return;
                    }
                    const createDatetimeUTC = (0, sonoff_1.parseIsoWithOffsetToUtcSeconds)(value.create_datetime);
                    if (createDatetimeUTC === undefined) {
                        logger_1.logger.error("irrigation_plan_settings invalid create_datetime, expected ISO 8601 datetime with timezone offset (Z or ±HH:mm).", NS);
                        return;
                    }
                    if (createDatetimeUTC < 0 || createDatetimeUTC > 0xffffffff) {
                        logger_1.logger.error("irrigation_plan_settings invalid create_datetime, converted unix out of uint32 range.", NS);
                        return;
                    }
                    ...
                    await utils
                        ...
                        .command(clusterName, "irrigationPlanSettings", { data: Array.from(payloadValue) }, defaultResponseOptions);
                    const state = { ...value };
                    if (SWVZNEFirmwareSupportsUnifiedImperialGallon(meta.device)) {
                        delete state.irrigation_amount_unit;
                    }
    ...
    irrigationPlanRemove: (endpointNames) => {
        const clusterName = "customClusterEwelink";
        const baseExposes = [
            e
                .composite("irrigation_plan_remove", "irrigation_plan_remove", ea.SET)
                .withDescription("Remove irrigation plan")
                .withFeature(e.numeric("plan_index", ea.SET).withValueMin(0).withValueMax(5).withDescription("The index of the irrigation plan to remove")),
        ];
        ...
        const toZigbee = [
            {
                ...
                convertSet: async (entity, key, value, meta) => {
                    ...
                    await utils
                        .determineEndpoint(entity, meta, clusterName)
                        .command(clusterName, "irrigationPlanRemove", { data: Array.from(data) }, { disableDefaultResponse: true });
    ...
    irrigationPlanRemovedReport: (endpointNames) => {
        const clusterName = "customClusterEwelink";
        const commandId = {
        ...
        const fromZigbee = [
            {
                ...
                convert: (model, msg, publish, options, meta) => {
                    ...
                    if (!parsedRawCommand)
                        return;
                    if (parsedRawCommand.commandId !== commandId.irrigationPlanRemovedReport)
                        return;
                    if (endpointNames) {
                        const endpointName = utils.getKey(model.endpoint?.(meta.device) ?? {}, msg.endpoint.ID);
                        if (!endpointName || !endpointNames.includes(endpointName))
                    ...
                    const payload = parsedRawCommand.payload;
                    if (payload.length < 1) {
                        logger_1.logger.error("irrigationPlanRemovedReport invalid payload length", NS);
                        return;
                    }
        ...
        const toZigbee = [
            {
                ...
                convertSet: async (entity, key, value, meta) => {
                    if (!utils.isString(value)) {
                        logger_1.logger.error("Invalid rain_delay, expected ISO 8601 datetime with timezone offset (Z or ±HH:mm).", NS);
                        return;
                    }
                    let delayEndTimeDevice;
                    let delayEndTimeUTC;
                    ...
                    if (value === "0") {
                        ...
                        delayEndTimeUTC = (0, sonoff_1.parseIsoWithOffsetToUtcSeconds)(value);
                        if (delayEndTimeUTC === undefined) {
                            logger_1.logger.error("Invalid rain_delay, expected ISO 8601 datetime with timezone offset (Z or ±HH:mm).", NS);
                            return;
                        }
                        const offsetSeconds = (0, sonoff_1.getRuntimeLocalOffsetSeconds)(Math.floor(Date.now() / 1000));
                        delayEndTimeDevice = (0, sonoff_1.utcToDeviceLocal2000Seconds)(delayEndTimeUTC, offsetSeconds);
                        stateValue = (0, sonoff_1.formatUtcSecondsToIsoWithOffset)(delayEndTimeUTC, offsetSeconds);
                    }
                    if (delayEndTimeDevice < 0 || delayEndTimeDevice > 0xffffffff) {
                        logger_1.logger.error("Invalid rain_delay, converted device 2000-local value out of uint32 range.", NS);
                        return;
                    }
                    const data = [
                    ...
                    await entity.command(clusterName, "rainDelay", { data }, { disableDefaultResponse: true });
    ...
    clearConsumptionHistory: () => {
        ...
        const clearPayload = { deviceType: 0x02, deviceLength: 0x01, eventType: 0x00 };
        const toZigbee = [
            {
                key: ["clear_history"],
                convertSet: async (entity, key, value, meta) => {
                    ...
                    await entity.command(clusterName, commandName, clearPayload, {
                        disableDefaultResponse: false,
                    });
    ...
    readConsumptionRecord(clusterName, commandName) {
        const exposes = [
        ...
        const fromZigbee = [
            {
                ...
                convert: (model, msg, publish, options, meta) => {
                    ...
                    if (status !== 0x00) {
                        logger_1.logger.warning(`readConsumptionRecord failed with status=${status}, subCmd=${subCmd}, offset=${offset}, recordIndex=${recordIndex}`, NS);
                        return;
                    }
                    if (subCmd === 0 && recordIndex === 240) {
                        ...
                        for (let i = 1; i + 5 < recordData.length; i += 6) {
                            ...
                            const usage = (recordData[i + 2] | (recordData[i + 3] << 8) | (recordData[i + 4] << 16) | (recordData[i + 5] << 24)) >>> 0;
                            if (usage !== 0xffffffff) {
                    ...
                    if (subCmd === 0) {
                        if (![0, 1].includes(recordIndex)) {
                            logger_1.logger.warning(`readConsumptionRecord invalid 24h recordIndex=${recordIndex}`, NS);
                            return;
                        }
                        const records = [];
                        ...
                        if (![0, 1].includes(recordIndex)) {
                            logger_1.logger.warning(`readConsumptionRecord invalid 30d recordIndex=${recordIndex}`, NS);
                            return;
                        }
                        const records = [];
                        const start = new Date();
                        ...
                        if (recordIndex !== 0) {
                            logger_1.logger.warning(`readConsumptionRecord invalid 180d recordIndex=${recordIndex}`, NS);
                            return;
                        }
                        const records = [];
                        ...
                        for (let i = 0; i < total; i++) {
                            const dataIndex = i * 4;
                            const usageRaw = (recordData[dataIndex] |
                    ...
                    if ((value.type === "day_of_hour" || value.type === "day_of_month") && typeof meta.state?.consumption_records === "string") {
                        try {
                            const existing = JSON.parse(meta.state.consumption_records);
        ...
        const toZigbee = [
            {
                ...
                convertSet: async (entity, key, value, meta) => {
                    ...
                    if (typeof metaOptions?.getLocalTime === "function") {
                        const localTime = metaOptions.getLocalTime();
                    ...
                    const payloadValue = [subCmd, offset & 0xff, index & 0xff, 0x00, 0x00, 0x00, 0x00];
                    ...
                    const sendReadRecord = async (recordIndex) => {
    ...
    faultCodeMiniZb1gsp: (args) => {
        const clusterName = "customClusterEwelink";
        const attributeName = "faultCode";
        const faultStates = [
        ...
        const fromZigbee = [
            {
                ...
                type: ["attributeReport"],
                convert: (model, msg) => {
                    if (msg.data.detachRelayActionEvent === undefined)
                        return;
    ...
    localFastSceneConfiguration: (args = {}) => {
        const clusterName = "customClusterEwelink";
        const attributeName = "localFastSceneConfiguration";
        const sceneProperty = "power_protector";
        ...
        const fromZigbee = [
            {
                ...
                type: ["attributeReport", "readResponse"],
                convert: (model, msg) => {
                    if (!(attributeName in msg.data))
                        return;
                    const bytes = (0, sonoff_1.zclArrayValueToBytes)(msg.data.localFastSceneConfiguration);
                    if (bytes === undefined)
                        return;
                    if (bytes.length < 3)
                        return;
                    ...
                    while (index + 1 < bytes.length) {
                        ...
                        index += length;
                        if (reportedSceneType !== powerProtectorType)
                            continue;
                        if (value[0] !== 1) {
                            logger_1.logger.error(`Local fast scene ${sceneProperty} is reported as disabled. Ignoring disabled state and reporting scene data.`, NS);
                        }
                        const maxVoltageProtect = (0, sonoff_1.readUInt32LE)(value, 10);
                        const minVoltageProtect = (0, sonoff_1.readUInt32LE)(value, 14);
                        const scene = {
                            max_current_protect: (0, sonoff_1.readUInt32LE)(value, 1) / 1000,
                            max_power_protect: (0, sonoff_1.readUInt32LE)(value, 5) / 1000,
        ...
        const toZigbee = [
            {
                ...
                convertSet: async (entity, key, value, meta) => {
                    ...
                    const getCachedBoolean = (property) => {
                        const cachedScene = meta.state?.[sceneProperty];
                        const cachedField = cachedScene?.[property];
                        return utils.isBoolean(cachedField) ? cachedField : false;
                    };
                    const getConfiguredBoolean = (property, supported) => supported ? getBoolean(property) : getCachedBoolean(property);
                    ...
                    const assertRange = (property, value, min, max) => {
                        if (value < min || value > max) {
                    ...
                    sceneValue.push(...(0, sonoff_1.toUInt32LEBytes)(Math.round(maxCurrentProtect * 1000)));
                    sceneValue.push(...(0, sonoff_1.toUInt32LEBytes)(Math.round(maxPowerProtect * 1000)));
                    ...
                    sceneValue.push(...(0, sonoff_1.toUInt32LEBytes)((getBoolean("max_voltage_protect_enabled") ? 0x80000000 : 0) | maxVoltage));
                    sceneValue.push(...(0, sonoff_1.toUInt32LEBytes)((getBoolean("min_voltage_protect_enabled") ? 0x80000000 : 0) | minVoltage));
                    ...
                    await entity.write(clusterName, {
                        [0x7016]: {
                            value: { elementType: zigbee_herdsman_1.Zcl.DataType.UINT8, elements: bytes },
                            type: zigbee_herdsman_1.Zcl.DataType.ARRAY,
                        },
                    }, utils.getOptions(meta.mapped, entity));
                    const state = { ...scene };
    ...
    temperatureHumidityCalculatedValues() {
        const calculate = (temperature, humidity) => {
        ...
        const fromZigbee = [
            {
                ...
                type: ["attributeReport", "readResponse"],
                convert: (model, msg, publish, options, meta) => {
                    if (msg.data.measuredValue === undefined || !utils.isNumber(meta.state.humidity))
                        return;
            ...
            },
            {
                ...
                type: ["attributeReport", "readResponse"],
                convert: (model, msg, publish, options, meta) => {
                    if (msg.data.measuredValue === undefined || !utils.isNumber(meta.state.temperature))
                        return;
    ...
    powerSupplyModeWithChangeBatteryState: () => {
        ...
        return m.enumLookup({
            ...
            fzConvert: (_model, msg, _publish, _options, meta) => {
                if (msg.data.powerSupplyMode === undefined)
                    return;
...
exports.definitions = [
    ...
    },
    {
        ...
        extend: [
            m.deviceAddCustomCluster("customSonoffSnzb02d", {
                ...
                ID: 0xfc11,
                attributes: {
                    comfortTemperatureMax: { name: "comfortTemperatureMax", ID: 0x0003, type: zigbee_herdsman_1.Zcl.DataType.INT16, write: true, min: -32768 },
                    comfortTemperatureMin: { name: "comfortTemperatureMin", ID: 0x0004, type: zigbee_herdsman_1.Zcl.DataType.INT16, write: true, min: -32768 },
                    comfortHumidityMin: { name: "comfortHumidityMin", ID: 0x0005, type: zigbee_herdsman_1.Zcl.DataType.UINT16, write: true, max: 0xffff },
                    comfortHumidityMax: { name: "comfortHumidityMax", ID: 0x0006, type: zigbee_herdsman_1.Zcl.DataType.UINT16, write: true, max: 0xffff },
                    temperatureUnits: { name: "temperatureUnits", ID: 0x0007, type: zigbee_herdsman_1.Zcl.DataType.UINT16, write: true, max: 0xffff },
                    temperatureCalibration: { name: "temperatureCalibration", ID: 0x2003, type: zigbee_herdsman_1.Zcl.DataType.INT16, write: true, min: -32768 },
                    humidityCalibration: { name: "humidityCalibration", ID: 0x2004, type: zigbee_herdsman_1.Zcl.DataType.INT16, write: true, min: -32768 },
                },
                commands: {},
                commandsResponse: {},
            }),
        ...
        extend: [
            m.deviceAddCustomCluster("customSonoffSnzb02dr2", {
                ...
                attributes: {
                    ...
                    temperatureCalibration: { name: "temperatureCalibration", ID: 0x2003, type: zigbee_herdsman_1.Zcl.DataType.INT16, write: true, min: -32768 },
                    humidityCalibration: { name: "humidityCalibration", ID: 0x2004, type: zigbee_herdsman_1.Zcl.DataType.INT16, write: true, min: -32768 },
                    temperatureSensorSelect: { name: "temperatureSensorSelect", ID: 0x600e, type: zigbee_herdsman_1.Zcl.DataType.UINT8, write: true, max: 0xff },
                    externalTemperature: { name: "externalTemperature", ID: 0x600d, type: zigbee_herdsman_1.Zcl.DataType.INT16, write: true, min: -32768 },
                    externalHumidity: { name: "externalHumidity", ID: 0x6018, type: zigbee_herdsman_1.Zcl.DataType.UINT16, write: true, max: 0xffff },
                },
                commands: {},
                commandsResponse: {},
            }),
            ...
            m.enumLookup({
                ...
                entityCategory: "config",
                description: "Data source shown on the display. Set to 'external' to enable the external display and show the values written to external_temperature and external_humidity; set to 'internal' to show the built-in sensor again.",
            }),
            m.numeric({
                name: "external_temperature",
                cluster: "customSonoffSnzb02dr2",
                attribute: "externalTemperature",
                description: "Temperature value to display when temperature_sensor_select is set to 'external'. Push readings here from another sensor (e.g. via an automation).",
                access: "STATE_SET",
                valueMin: -50,
                valueMax: 125,
                scale: 100,
            ...
            m.numeric({
                ...
                description: "Relative humidity value to display when temperature_sensor_select is set to 'external'. Push readings here from another sensor. Requires device firmware 1.0.4 or later.",
                access: "STATE_SET",
                valueMin: 0,
                valueMax: 100,
                scale: 100,
    ...
    },
    {
        ...
        extend: [
            ...
            m.deviceAddCustomCluster("customSonoffSnzb01m", {
                ...
                ID: 0xfc12,
                attributes: {
                    keyActionEvent: { name: "keyActionEvent", ID: 0x0000, type: zigbee_herdsman_1.Zcl.DataType.UINT8, write: true, max: 0xff },
                },
                commands: {},
                commandsResponse: {},
            }),
        ...
        extend: [
            ...
            m.deviceAddCustomCluster("customSonoffTrvzb", {
                ...
                attributes: {
                    ...
                    tamper: { name: "tamper", ID: 0x2000, type: zigbee_herdsman_1.Zcl.DataType.UINT8, write: true, max: 0xff },
                    illumination: { name: "illumination", ID: 0x2001, type: zigbee_herdsman_1.Zcl.DataType.UINT8, write: true, max: 0xff },
                    openWindow: { name: "openWindow", ID: 0x6000, type: zigbee_herdsman_1.Zcl.DataType.BOOLEAN, write: true },
                    frostProtectionTemperature: { name: "frostProtectionTemperature", ID: 0x6002, type: zigbee_herdsman_1.Zcl.DataType.INT16, write: true, min: -32768 },
                    idleSteps: { name: "idleSteps", ID: 0x6003, type: zigbee_herdsman_1.Zcl.DataType.UINT16, write: true, max: 0xffff },
                    closingSteps: { name: "closingSteps", ID: 0x6004, type: zigbee_herdsman_1.Zcl.DataType.UINT16, write: true, max: 0xffff },
                    valveOpeningLimitVoltage: { name: "valveOpeningLimitVoltage", ID: 0x6005, type: zigbee_herdsman_1.Zcl.DataType.UINT16, write: true, max: 0xffff },
                    valveClosingLimitVoltage: { name: "valveClosingLimitVoltage", ID: 0x6006, type: zigbee_herdsman_1.Zcl.DataType.UINT16, write: true, max: 0xffff },
                    valveMotorRunningVoltage: { name: "valveMotorRunningVoltage", ID: 0x6007, type: zigbee_herdsman_1.Zcl.DataType.UINT16, write: true, max: 0xffff },
                    valveOpeningDegree: { name: "valveOpeningDegree", ID: 0x600b, type: zigbee_herdsman_1.Zcl.DataType.UINT8, write: true, max: 0xff },
                    valveClosingDegree: { name: "valveClosingDegree", ID: 0x600c, type: zigbee_herdsman_1.Zcl.DataType.UINT8, write: true, max: 0xff },
                    tempAccuracy: { name: "tempAccuracy", ID: 0x6011, type: zigbee_herdsman_1.Zcl.DataType.INT16, write: true, min: -32768 },
                    temporaryMode: { name: "temporaryMode", ID: 0x6014, type: zigbee_herdsman_1.Zcl.DataType.UINT8, write: true, max: 0xff },
                    temporaryModeTime: { name: "temporaryModeTime", ID: 0x6015, type: zigbee_herdsman_1.Zcl.DataType.UINT32, write: true, max: 0xffffffff },
                    temporaryModeTemp: { name: "temporaryModeTemp", ID: 0x6016, type: zigbee_herdsman_1.Zcl.DataType.INT16, write: true, min: -32768 },
                    ...
                    smartTempControl: { name: "smartTempControl", ID: 0x6017, type: zigbee_herdsman_1.Zcl.DataType.BITMAP8, write: true },
                },
                commands: {},
                commandsResponse: {},
            }),
            ...
            m.binary({
                ...
                description: "Enable adaptive valve control using a PID algorithm. " +
    ...
    },
    {
        ...
        exposes: [
            e
                .climate()
                .withSetpoint("occupied_heating_setpoint", sonoffTrvzbtTargetTemperatureRange.min, sonoffTrvzbtTargetTemperatureRange.max, sonoffTrvzbtTargetTemperatureRange.step)
                .withLocalTemperature()
                .withLocalTemperatureCalibration(sonoffTrvzbtLocalTemperatureCalibrationRange.min, sonoffTrvzbtLocalTemperatureCalibrationRange.max, sonoffTrvzbtLocalTemperatureCalibrationRange.step)
                .withSystemMode(["off", "auto", "heat"], ea.ALL, "Mode of the thermostat")
                .withRunningState(["idle", "heat"], ea.STATE_GET),
            e.battery(),
        ],
        ...
        extend: [
            ...
            m.deviceAddCustomCluster("customSonoffTrvzbt", {
                ...
                attributes: {
                    childLock: { name: "childLock", ID: 0x0000, type: zigbee_herdsman_1.Zcl.DataType.BOOLEAN, write: true },
                    faultCode: { name: "faultCode", ID: 0x0010, type: zigbee_herdsman_1.Zcl.DataType.UINT32, max: 0xffffffff },
                    screenDirection: { name: "screenDirection", ID: 0x0021, type: zigbee_herdsman_1.Zcl.DataType.UINT8, write: true, max: 0xff },
                    openWindow: { name: "openWindow", ID: 0x6000, type: zigbee_herdsman_1.Zcl.DataType.BOOLEAN, write: true },
                    frostProtectionTemperature: { name: "frostProtectionTemperature", ID: 0x6002, type: zigbee_herdsman_1.Zcl.DataType.INT16, write: true, min: -32768 },
                    idleSteps: { name: "idleSteps", ID: 0x6003, type: zigbee_herdsman_1.Zcl.DataType.UINT16, max: 0xffff },
                    closingSteps: { name: "closingSteps", ID: 0x6004, type: zigbee_herdsman_1.Zcl.DataType.UINT16, max: 0xffff },
                    valveOpeningLimitVoltage: { name: "valveOpeningLimitVoltage", ID: 0x6005, type: zigbee_herdsman_1.Zcl.DataType.UINT16, max: 0xffff },
                    valveClosingLimitVoltage: { name: "valveClosingLimitVoltage", ID: 0x6006, type: zigbee_herdsman_1.Zcl.DataType.UINT16, max: 0xffff },
                    valveMotorRunningVoltage: { name: "valveMotorRunningVoltage", ID: 0x6007, type: zigbee_herdsman_1.Zcl.DataType.UINT16, max: 0xffff },
                    valveOpeningDegree: { name: "valveOpeningDegree", ID: 0x600b, type: zigbee_herdsman_1.Zcl.DataType.UINT8, write: true, max: 0xff },
                    valveClosingDegree: { name: "valveClosingDegree", ID: 0x600c, type: zigbee_herdsman_1.Zcl.DataType.UINT8, write: true, max: 0xff },
                    externalTemperatureInput: { name: "externalTemperatureInput", ID: 0x600d, type: zigbee_herdsman_1.Zcl.DataType.INT16, write: true, min: -32768 },
                    temperatureSensorSelect: { name: "temperatureSensorSelect", ID: 0x600e, type: zigbee_herdsman_1.Zcl.DataType.UINT8, write: true, max: 0xff },
                    ...
                    temperatureControlMode: { name: "temperatureControlMode", ID: 0x6013, type: zigbee_herdsman_1.Zcl.DataType.ENUM8, write: true, max: 0xff },
                    temporaryMode: { name: "temporaryMode", ID: 0x6014, type: zigbee_herdsman_1.Zcl.DataType.UINT8, write: true, max: 0xff },
                    temporaryModeTime: { name: "temporaryModeTime", ID: 0x6015, type: zigbee_herdsman_1.Zcl.DataType.UINT32, write: true, max: 0xffffffff },
                    temporaryModeTemp: { name: "temporaryModeTemp", ID: 0x6016, type: zigbee_herdsman_1.Zcl.DataType.INT16, write: true, min: -32768 },
                    lowBatteryValveState: { name: "lowBatteryValveState", ID: 0x601c, type: zigbee_herdsman_1.Zcl.DataType.UINT8, write: true, max: 0xff },
                    weeklyScheduleActiveNum: { name: "weeklyScheduleActiveNum", ID: 0x601d, type: zigbee_herdsman_1.Zcl.DataType.UINT8, write: true, max: 0xff },
                    hvacMessageNotification: { name: "hvacMessageNotification", ID: 0x6030, type: zigbee_herdsman_1.Zcl.DataType.ARRAY },
                    heatPercentageHour: { name: "heatPercentageHour", ID: 0x6033, type: zigbee_herdsman_1.Zcl.DataType.UINT8, max: 0xff },
                    motorTravelCalibration: { name: "motorTravelCalibration", ID: 0x6036, type: zigbee_herdsman_1.Zcl.DataType.BOOLEAN, write: true },
                    motorTravelCalibrationStatus: { name: "motorTravelCalibrationStatus", ID: 0x6037, type: zigbee_herdsman_1.Zcl.DataType.UINT8, max: 0xff },
                },
                commands: {
                    ...
                    bluetoothPairing: { name: "bluetoothPairing", ID: 0x10, parameters: [{ name: "data", type: zigbee_herdsman_1.Zcl.BuffaloZclDataType.LIST_UINT8 }] },
                    scheduleGroup: { name: "scheduleGroup", ID: 0x13, parameters: [{ name: "data", type: zigbee_herdsman_1.Zcl.BuffaloZclDataType.LIST_UINT8 }] },
                },
                commandsResponse: {},
            }),
            ...
            m.numeric({
                ...
                attribute: "valveClosingDegree",
                entityCategory: "config",
                description: "Valve opening percentage when not heating.Recommended: set the heating valve position higher than the idle valve position.",
                valueMin: 0,
                valueMax: 100,
                valueStep: 1,
                unit: "%",
        ...
        configure: async (device, coordinatorEndpoint) => {
            ...
            const readCustomAttributes = async (attributes) => {
                try {
                    await endpoint.read(0xfc11, attributes);
                }
                catch (error) {
                    if (attributes.length === 1) {
                        logger_1.logger.error(`TRV-ZBT failed to read private attribute 0x${attributes[0].toString(16)}: ${error}`, NS);
                        return;
                    }
    ...
    },
    {
        ...
        exposes: [
            e
                .climate()
                .withSetpoint("occupied_heating_setpoint", sonoffTpWgzbaTargetTemperatureRange.min, sonoffTpWgzbaTargetTemperatureRange.max, sonoffTpWgzbaTargetTemperatureRange.step)
                .withLocalTemperature()
                .withLocalTemperatureCalibration(sonoffTpWgzbaLocalTemperatureCalibrationRange.min, sonoffTpWgzbaLocalTemperatureCalibrationRange.max, sonoffTpWgzbaLocalTemperatureCalibrationRange.step)
                .withSystemMode(["off", "auto", "heat"], ea.ALL, "Mode of the thermostat")
                .withRunningState(["idle", "heat"], ea.STATE_GET),
        ],
        ...
        extend: [
            ...
            m.deviceAddCustomCluster("customSonoffTpWgzba", {
                ...
                attributes: {
                    childLock: { name: "childLock", ID: 0x0000, type: zigbee_herdsman_1.Zcl.DataType.BOOLEAN, write: true },
                    btPairingBroadcastReq: { name: "btPairingBroadcastReq", ID: 0x0029, type: zigbee_herdsman_1.Zcl.DataType.UINT8, write: true, max: 0xff },
                    openWindow: { name: "openWindow", ID: 0x6000, type: zigbee_herdsman_1.Zcl.DataType.BOOLEAN, write: true },
                    frostProtectionTemperature: { name: "frostProtectionTemperature", ID: 0x6002, type: zigbee_herdsman_1.Zcl.DataType.INT16, write: true, min: -32768 },
                    temporaryMode: { name: "temporaryMode", ID: 0x6014, type: zigbee_herdsman_1.Zcl.DataType.UINT8, write: true, max: 0xff },
                    temporaryModeTime: { name: "temporaryModeTime", ID: 0x6015, type: zigbee_herdsman_1.Zcl.DataType.UINT32, write: true, max: 0xffffffff },
                    ...
                    weeklyScheduleActiveNum: { name: "weeklyScheduleActiveNum", ID: 0x601d, type: zigbee_herdsman_1.Zcl.DataType.UINT8, write: true, max: 0xff },
                    hvacMessageNotification: { name: "hvacMessageNotification", ID: 0x6030, type: zigbee_herdsman_1.Zcl.DataType.ARRAY },
                    temperatureControlThreshold: { name: "temperatureControlThreshold", ID: 0x601f, type: zigbee_herdsman_1.Zcl.DataType.STRUCT, write: true },
                    radarSensitivityLevel: { name: "radarSensitivityLevel", ID: 0x6020, type: zigbee_herdsman_1.Zcl.DataType.UINT8, write: true, max: 0xff },
                    radarDoNotDisturbEnable: { name: "radarDoNotDisturbEnable", ID: 0x6021, type: zigbee_herdsman_1.Zcl.DataType.BOOLEAN, write: true },
                    radarDoNotDisturbPeriod: { name: "radarDoNotDisturbPeriod", ID: 0x6022, type: zigbee_herdsman_1.Zcl.DataType.STRUCT, write: true },
                    screenWorkingBrightness: { name: "screenWorkingBrightness", ID: 0x6023, type: zigbee_herdsman_1.Zcl.DataType.UINT8, write: true, max: 0xff },
                    screenStandbyBrightness: { name: "screenStandbyBrightness", ID: 0x6024, type: zigbee_herdsman_1.Zcl.DataType.UINT8, write: true, max: 0xff },
                    ...
                    screenNightModeEnable: { name: "screenNightModeEnable", ID: 0x6026, type: zigbee_herdsman_1.Zcl.DataType.BOOLEAN, write: true },
                    screenNightModePeriod: { name: "screenNightModePeriod", ID: 0x6027, type: zigbee_herdsman_1.Zcl.DataType.STRUCT, write: true },
                    relayOutputType: { name: "relayOutputType", ID: 0x6028, type: zigbee_herdsman_1.Zcl.DataType.BITMAP8, write: true, max: 0xff },
                    remoteAttributeLinkage: { name: "remoteAttributeLinkage", ID: 0x601e, type: zigbee_herdsman_1.Zcl.DataType.ARRAY, write: true },
                    currentNtcTemperature: { name: "currentNtcTemperature", ID: 0x6031, type: zigbee_herdsman_1.Zcl.DataType.INT16, min: -32768 },
                    ...
                    overheatProtectionEnable: { name: "overheatProtectionEnable", ID: 0x6034, type: zigbee_herdsman_1.Zcl.DataType.BOOLEAN, write: true },
                    radarEnable: { name: "radarEnable", ID: 0x6035, type: zigbee_herdsman_1.Zcl.DataType.BOOLEAN, write: true },
                },
                commands: {
                    setTemporaryMode: { name: "setTemporaryMode", ID: 0x11, parameters: [{ name: "data", type: zigbee_herdsman_1.Zcl.BuffaloZclDataType.LIST_UINT8 }] },
                    readTemperatureControlHistory: {
                        name: "readTemperatureControlHistory",
                        ID: 0x0e,
                        parameters: [{ name: "data", type: zigbee_herdsman_1.Zcl.BuffaloZclDataType.LIST_UINT8 }],
            ...
            sonoffExtend.tpWgzbaTimePeriod("radar_do_not_disturb_period", "radarDoNotDisturbPeriod", "During the scheduled period, presence detection will not wake the screen automatically."),
            m.numeric({
        ...
        configure: async (device, coordinatorEndpoint) => {
            ...
            const readCustomAttributes = async (attributes) => {
                try {
                    ...
                    if (attributes.length === 1) {
                        logger_1.logger.error(`TP-WGZBA failed to read private attribute 0x${attributes[0].toString(16)}: ${error}`, NS);
                        return;
                    }
                    for (const attribute of attributes) {
                        await readCustomAttributes([attribute]);
    ...
    },
    {
        ...
        extend: [
            ...
            m.numeric({
                ...
                cluster: "customClusterEwelink",
                attribute: { ID: 0x5006, type: zigbee_herdsman_1.Zcl.DataType.UINT32 },
            ...
            m.numeric({
                ...
                cluster: "customClusterEwelink",
                attribute: { ID: 0x5007, type: zigbee_herdsman_1.Zcl.DataType.UINT32 },
            ...
            m.numeric({
                ...
                cluster: "customClusterEwelink",
                attribute: { ID: 0x500d, type: zigbee_herdsman_1.Zcl.DataType.UINT32 },
            ...
            m.numeric({
                ...
                cluster: "customClusterEwelink",
                attribute: { ID: 0x500e, type: zigbee_herdsman_1.Zcl.DataType.UINT32 },
            ...
            m.numeric({
                ...
                cluster: "customClusterEwelink",
                attribute: { ID: 0x500f, type: zigbee_herdsman_1.Zcl.DataType.UINT32 },
            ...
            m.binary({
                ...
                cluster: "customClusterEwelink",
                attribute: { ID: 0x5010, type: zigbee_herdsman_1.Zcl.DataType.BOOLEAN },
                description: "Current valve work state, this turns on when a Cyclic timed or quantitative irrigation program is running, and turns off when the program is complete.",
                access: "STATE",
                valueOn: ["working", 1],
                valueOff: ["idle", 0],
            }),
        ...
        configure: async (device, coordinatorEndpoint) => {
            ...
            await endpoint.read("customClusterEwelink", [0x500c]).catch((error) => {
                logger_1.logger.warning(`SWV read customClusterEwelink(current_device_status) failed: ${error}`, NS);
            });
            await endpoint.read("customClusterEwelink", [0x5011]).catch((error) => {
                logger_1.logger.warning(`SWV read customClusterEwelink(lackWaterCloseValveTimeout) failed: ${error}`, NS);
            });
        },
    },
    {
    ...
    },
    {
        ...
        configure: async (device, coordinatorEndpoint) => {
            const endpoint = device.getEndpoint(1);
            await endpoint.read("customClusterEwelink", [0x0016, 0x001e, 0x4001, 0x4002, 0x4003, 0x4006], defaultResponseOptions);
        },
    },
    {
        ...
        extend: [
            m.deviceAddCustomCluster("customClusterEwelink", {
                ...
                attributes: {
                    childLock: { name: "childLock", ID: 0x0000, type: zigbee_herdsman_1.Zcl.DataType.BOOLEAN, write: true },
                    realTimeIrrigationDuration: { name: "realTimeIrrigationDuration", ID: 0x5006, type: zigbee_herdsman_1.Zcl.DataType.UINT32 },
                    realTimeIrrigationVolume: { name: "realTimeIrrigationVolume", ID: 0x5007, type: zigbee_herdsman_1.Zcl.DataType.UINT32 },
                    valveAbnormalState: { name: "valveAbnormalState", ID: 0x500c, type: zigbee_herdsman_1.Zcl.DataType.UINT8 },
                    rainDelayEndDatetime: { name: "rainDelayEndDatetime", ID: 0x5014, type: zigbee_herdsman_1.Zcl.DataType.UINT32 },
                    hourIrrigationVolume: { name: "hourIrrigationVolume", ID: 0x501b, type: zigbee_herdsman_1.Zcl.DataType.UINT32 },
                    hourIrrigationDuration: { name: "hourIrrigationDuration", ID: 0x501c, type: zigbee_herdsman_1.Zcl.DataType.UINT32 },
                    manualDefaultSettings: { name: "manualDefaultSettings", ID: 0x501d, type: zigbee_herdsman_1.Zcl.DataType.ARRAY, write: true },
                    seasonalWateringAdjustment: { name: "seasonalWateringAdjustment", ID: 0x501e, type: zigbee_herdsman_1.Zcl.DataType.ARRAY, write: true },
                    irrigationScheduleStatus: { name: "irrigationScheduleStatus", ID: 0x501f, type: zigbee_herdsman_1.Zcl.DataType.ARRAY },
                    valveAlarmSettings: { name: "valveAlarmSettings", ID: 0x5020, type: zigbee_herdsman_1.Zcl.DataType.ARRAY, write: true },
                    unitOfWaterFlow: { name: "unitOfWaterFlow", ID: 0x5021, type: zigbee_herdsman_1.Zcl.DataType.UINT8, write: true },
                },
                commands: {
                    readRecord: { name: "readRecord", ID: 0x00, parameters: [{ name: "data", type: zigbee_herdsman_1.Zcl.BuffaloZclDataType.LIST_UINT8 }] },
                    irrigationPlanSettings: {
                    ...
                    rainDelay: { name: "rainDelay", ID: 0x08, parameters: [{ name: "data", type: zigbee_herdsman_1.Zcl.BuffaloZclDataType.LIST_UINT8 }] },
                },
                commandsResponse: {
        ...
        configure: async (device, coordinatorEndpoint) => {
            ...
            if (endpoint) {
                try {
                    ...
                    logger_1.logger.warning(`SWV-ZN genOnOff bind/reporting failed, continuing without reporting: ${error}`, NS);
                }
                await endpoint.read("genOnOff", ["onOff"]).catch((error) => {
                    logger_1.logger.warning(`SWV-ZN read genOnOff.onOff failed: ${error}`, NS);
                });
                if (SWVZNEFirmwareSupportsUnifiedImperialGallon(device)) {
                    await endpoint.read("customClusterEwelink", ["unitOfWaterFlow"]).catch((error) => {
                        logger_1.logger.warning(`SWV-ZN read customClusterEwelink.unitOfWaterFlow failed: ${error}`, NS);
                    });
                }
            }
        },
    },
    {
        ...
        configure: async (device, coordinatorEndpoint) => {
            for (const endpointId of [1, 2]) {
                ...
                if (!endpoint) {
                    logger_1.logger.warning(`SWV-ZF2 endpoint ${endpointId} not found`, NS);
                    continue;
                }
                try {
                    await reporting.bind(endpoint, coordinatorEndpoint, ["genOnOff"]);
                    ...
                    logger_1.logger.warning(`SWV-ZF2 genOnOff bind/reporting failed on endpoint ${endpointId}, continuing without reporting: ${error}`, NS);
                }
                await endpoint.read("genOnOff", ["onOff"]).catch((error) => {
                    logger_1.logger.warning(`SWV-ZF2 read genOnOff.onOff failed on endpoint ${endpointId}: ${error}`, NS);
                });
            }
            const endpoint = device.getEndpoint(1);
            if (endpoint) {
                await endpoint.read("customClusterEwelink", [0x500c]).catch((error) => {
                    logger_1.logger.warning(`SWV-ZF2 read customClusterEwelink(valveAbnormalState) failed: ${error}`, NS);
                });
                if (SWVZNEFirmwareSupportsUnifiedImperialGallon(device)) {
                    await endpoint.read("customClusterEwelink", ["unitOfWaterFlow"]).catch((error) => {
                        logger_1.logger.warning(`SWV-ZF2 read customClusterEwelink.unitOfWaterFlow failed: ${error}`, NS);
                    });
                }
            }
        },
    ...
    },
    {
        ...
        extend: [
            m.deviceAddCustomCluster("customClusterEwelink", {
                ...
                ID: 0xfc11,
                attributes: {
                    temperatureCalibration: { name: "temperatureCalibration", ID: 0x2003, type: zigbee_herdsman_1.Zcl.DataType.INT16, write: true },
                    humidityCalibration: { name: "humidityCalibration", ID: 0x2004, type: zigbee_herdsman_1.Zcl.DataType.INT16, write: true },
                },
                commands: {},
                commandsResponse: {},
            }),
            ...
            m.numeric({
                ...
                attribute: "temperatureCalibration",
                entityCategory: "config",
                description: "Offset to add/subtract to the reported temperature. Note: wake up the device by pressing the button on the back before changing this value.",
                valueMin: -50,
                valueMax: 50,
                scale: 100,
                valueStep: 0.1,
            ...
            m.numeric({
                ...
                attribute: "humidityCalibration",
                entityCategory: "config",
                description: "Offset to add/subtract to the reported relative humidity. Note: wake up the device by pressing the button on the back before changing this value.",
                valueMin: -50,
                valueMax: 50,
                scale: 100,
                valueStep: 0.1,
        ...
        extend: [
            m.deviceAddCustomCluster("customClusterEwelink", {
                ...
                attributes: {
                    temperatureCalibration: { name: "temperatureCalibration", ID: 0x2003, type: zigbee_herdsman_1.Zcl.DataType.INT16, write: true },
                    humidityCalibration: { name: "humidityCalibration", ID: 0x2004, type: zigbee_herdsman_1.Zcl.DataType.INT16, write: true },
                    pressureCalibration: { name: "pressureCalibration", ID: 0x2007, type: zigbee_herdsman_1.Zcl.DataType.INT16, write: true },
                },
                commands: {},
                commandsResponse: {},
            }),
            ...
            m.numeric({
                ...
                attribute: { ID: 0x0004, type: zigbee_herdsman_1.Zcl.DataType.INT32 },
                access: "STATE_GET",
            ...
            m.numeric({
                ...
                attribute: "temperatureCalibration",
                entityCategory: "config",
                description: "Calibrated temperature target value (supports 0.1°C step). Note: wake up the device by pressing the button on the back before changing this value.",
                valueMin: -20,
                valueMax: 60,
                scale: 100,
                valueStep: 0.1,
            ...
            m.numeric({
                ...
                attribute: "humidityCalibration",
                entityCategory: "config",
                description: "Calibrated relative humidity target value (supports 0.1% step). Note: wake up the device by pressing the button on the back before changing this value.",
                valueMin: 5,
                valueMax: 95,
                scale: 100,
                valueStep: 0.1,
            ...
            m.numeric({
                ...
                entityCategory: "config",
                description: "Pressure compensation offset applied directly to pressure reading in hPa (positive adds, negative subtracts). Range: -400 to 400 hPa. " +
    ...
    },
    {
        ...
        extend: [
            m.deviceAddCustomCluster("customClusterEwelink", {
                ...
                attributes: {
                    networkLed: { name: "networkLed", ID: 0x0001, type: zigbee_herdsman_1.Zcl.DataType.BOOLEAN, write: true },
                    faultCode: { name: "faultCode", ID: 0x0010, type: zigbee_herdsman_1.Zcl.DataType.UINT32, max: 0xffffffff },
                    limitsOfThresholdValue: { name: "limitsOfThresholdValue", ID: 0x7003, type: zigbee_herdsman_1.Zcl.DataType.CHAR_STR, write: true },
                    acCurrentCurrentValue: { name: "acCurrentCurrentValue", ID: 0x7004, type: zigbee_herdsman_1.Zcl.DataType.UINT32, max: 0xffffffff },
                    acCurrentVoltageValue: { name: "acCurrentVoltageValue", ID: 0x7005, type: zigbee_herdsman_1.Zcl.DataType.UINT32, max: 0xffffffff },
                    acCurrentPowerValue: { name: "acCurrentPowerValue", ID: 0x7006, type: zigbee_herdsman_1.Zcl.DataType.UINT32, max: 0xffffffff },
                    outlet_control_protect: { name: "outlet_control_protect", ID: 0x7007, type: zigbee_herdsman_1.Zcl.DataType.UINT8, write: true, max: 0xff },
                    energyToday: { name: "energyToday", ID: 0x7009, type: zigbee_herdsman_1.Zcl.DataType.UINT32, max: 0xffffffff },
                    energyMonth: { name: "energyMonth", ID: 0x700a, type: zigbee_herdsman_1.Zcl.DataType.UINT32, max: 0xffffffff },
                    energyYesterday: { name: "energyYesterday", ID: 0x700b, type: zigbee_herdsman_1.Zcl.DataType.UINT32, max: 0xffffffff },
                    acCurrentMaxOverloadEnable: { name: "acCurrentMaxOverloadEnable", ID: 0x700c, type: zigbee_herdsman_1.Zcl.DataType.UINT8, write: true, max: 0xff },
                    acCurrentMaxOverload: { name: "acCurrentMaxOverload", ID: 0x700d, type: zigbee_herdsman_1.Zcl.DataType.UINT32, write: true, max: 0xffffffff },
                    acVoltageMaxOverloadEnable: { name: "acVoltageMaxOverloadEnable", ID: 0x700e, type: zigbee_herdsman_1.Zcl.DataType.UINT8, write: true, max: 0xff },
                    acVoltageMaxOverload: { name: "acVoltageMaxOverload", ID: 0x700f, type: zigbee_herdsman_1.Zcl.DataType.UINT32, write: true, max: 0xffffffff },
                    acPowerMaxOverloadEnable: { name: "acPowerMaxOverloadEnable", ID: 0x7010, type: zigbee_herdsman_1.Zcl.DataType.UINT8, write: true, max: 0xff },
                    acPowerMaxOverload: { name: "acPowerMaxOverload", ID: 0x7011, type: zigbee_herdsman_1.Zcl.DataType.UINT32, write: true, max: 0xffffffff },
                    totalEnergyConsumption: { name: "totalEnergyConsumption", ID: 0x701e, type: zigbee_herdsman_1.Zcl.DataType.UINT32, max: 0xffffffff },
                },
                commands: {
                    protocolData: { name: "protocolData", ID: 0x01, parameters: [{ name: "data", type: zigbee_herdsman_1.Zcl.BuffaloZclDataType.LIST_UINT8 }] },
                    clearHistory: {
                        ...
                        ID: 0x0c,
                        parameters: [
                            { name: "deviceType", type: zigbee_herdsman_1.Zcl.DataType.UINT8 },
                            { name: "deviceLength", type: zigbee_herdsman_1.Zcl.DataType.UINT8 },
                            { name: "eventType", type: zigbee_herdsman_1.Zcl.DataType.UINT8 },
                        ],
                    },
                    readRecord: { name: "readRecord", ID: 0x02, parameters: [{ name: "data", type: zigbee_herdsman_1.Zcl.BuffaloZclDataType.LIST_UINT8 }] },
                },
                commandsResponse: {},
            }),
            m.onOff({
            ...
            m.numeric({
                ...
                fzConvert: (model, msg) => {
                    ...
                    const multiplier = msg.endpoint.getClusterAttributeValue("seMetering", "multiplier") || 1;
                    const divisor = msg.endpoint.getClusterAttributeValue("seMetering", "divisor") || 1000;
        ...
        configure: async (device, coordinatorEndpoint) => {
            ...
            await reporting.bind(endpoint, coordinatorEndpoint, ["genOnOff", "customClusterEwelink", "seMetering"]);
            await reporting.onOff(endpoint, { min: 1, max: 1800, change: 0 });
            await endpoint.read("customClusterEwelink", ["acCurrentCurrentValue", "acCurrentVoltageValue", "acCurrentPowerValue", 0x7003, "outlet_control_protect", "totalEnergyConsumption"], defaultResponseOptions);
            await endpoint.configureReporting("customClusterEwelink", [
                { attribute: "energyMonth", minimumReportInterval: 60, maximumReportInterval: 3600, reportableChange: 50 },
                { attribute: "energyYesterday", minimumReportInterval: 60, maximumReportInterval: 3600, reportableChange: 50 },
                { attribute: "energyToday", minimumReportInterval: 60, maximumReportInterval: 3600, reportableChange: 50 },
    ...
    },
    {
        ...
        extend: [
            m.deviceAddCustomCluster("customClusterEwelink", {
                ...
                ID: 0xfc11,
                attributes: {
                    occupancyZoneEnable: { name: "occupancyZoneEnable", ID: 0x2016, type: zigbee_herdsman_1.Zcl.DataType.BITMAP16, write: true },
                ...
                commands: {
                    spatialLearning: { name: "spatialLearning", ID: 0x04, parameters: [{ name: "data", type: zigbee_herdsman_1.Zcl.BuffaloZclDataType.LIST_UINT8 }] },
                },
                commandsResponse: {},
            }),
            // official cluster
        ...
        configure: async (device, coordinatorEndpoint) => {
            ...
            if (endpoint) {
                ...
                await endpoint.read("customClusterEwelink", ["occupancyZoneEnable"]).catch((error) => {
                    logger_1.logger.warning(`SNZB-06P24 configure: read occupancyZoneEnable failed, ${error}`, NS);
                });
            }
        },
    },
    {
        ...
        extend: [
            m.deviceAddCustomCluster("customClusterEwelink", {
                ...
                attributes: {
                    networkLed: { name: "networkLed", ID: 0x0001, type: zigbee_herdsman_1.Zcl.DataType.BOOLEAN, write: true },
                    faultCode: { name: "faultCode", ID: 0x0010, type: zigbee_herdsman_1.Zcl.DataType.UINT32 },
                    radioPower: { name: "radioPower", ID: 0x0012, type: zigbee_herdsman_1.Zcl.DataType.INT16, write: true },
                    delayedPowerOnState: { name: "delayedPowerOnState", ID: 0x0014, type: zigbee_herdsman_1.Zcl.DataType.BOOLEAN, write: true },
                    delayedPowerOnTime: { name: "delayedPowerOnTime", ID: 0x0015, type: zigbee_herdsman_1.Zcl.DataType.UINT16, write: true },
                    externalTriggerMode: { name: "externalTriggerMode", ID: 0x0016, type: zigbee_herdsman_1.Zcl.DataType.UINT8, write: true },
                    detachRelayMode2: { name: "detachRelayMode2", ID: 0x0019, type: zigbee_herdsman_1.Zcl.DataType.BITMAP8, write: true },
                    detachRelayActionEvent: { name: "detachRelayActionEvent", ID: 0x0028, type: zigbee_herdsman_1.Zcl.DataType.UINT8 },
                    acCurrentCurrentValue: { name: "acCurrentCurrentValue", ID: 0x7004, type: zigbee_herdsman_1.Zcl.DataType.UINT32 },
                    acCurrentVoltageValue: { name: "acCurrentVoltageValue", ID: 0x7005, type: zigbee_herdsman_1.Zcl.DataType.UINT32 },
                    acCurrentPowerValue: { name: "acCurrentPowerValue", ID: 0x7006, type: zigbee_herdsman_1.Zcl.DataType.UINT32 },
                    energyToday: { name: "energyToday", ID: 0x7009, type: zigbee_herdsman_1.Zcl.DataType.UINT32 },
                    energyMonth: { name: "energyMonth", ID: 0x700a, type: zigbee_herdsman_1.Zcl.DataType.UINT32 },
                    energyYesterday: { name: "energyYesterday", ID: 0x700b, type: zigbee_herdsman_1.Zcl.DataType.UINT32 },
                    localFastSceneConfiguration: { name: "localFastSceneConfiguration", ID: 0x7016, type: zigbee_herdsman_1.Zcl.DataType.ARRAY, write: true },
                    outputEnergyToday: { name: "outputEnergyToday", ID: 0x7018, type: zigbee_herdsman_1.Zcl.DataType.UINT32 },
                    outputEnergyMonth: { name: "outputEnergyMonth", ID: 0x7019, type: zigbee_herdsman_1.Zcl.DataType.UINT32 },
                    totalEnergyConsumption: { name: "totalEnergyConsumption", ID: 0x701e, type: zigbee_herdsman_1.Zcl.DataType.UINT32 },
                    totalOutputEnergyConsumption: { name: "totalOutputEnergyConsumption", ID: 0x701f, type: zigbee_herdsman_1.Zcl.DataType.UINT32 },
                },
                commands: {
                    protocolData: { name: "protocolData", ID: 0x01, parameters: [{ name: "data", type: zigbee_herdsman_1.Zcl.BuffaloZclDataType.LIST_UINT8 }] },
                },
            ...
            m.numeric({
                ...
                fzConvert: (model, msg, publish, options, meta) => {
                    if ("acCurrentPowerValue" in msg.data) {
                        return { power: (0, sonoff_1.signedInt32MilliToValue)(msg.data.acCurrentPowerValue) };
                    }
                },
            }),
            m.numeric({
        ...
        configure: async (device, coordinatorEndpoint) => {
            const endpoint = device.getEndpoint(1);
            await reporting.bind(endpoint, coordinatorEndpoint, ["genOnOff", "customClusterEwelink"]);
            await endpoint.read("customClusterEwelink", ["faultCode"], defaultResponseOptions);
        },
    },
    {
    ...
    },
    {
        ...
        extend: [
            m.deviceAddCustomCluster("customClusterEwelink", {
                ...
                ID: 0xfc11,
                attributes: {
                    powerSupplyMode: { name: "powerSupplyMode", ID: 0x0024, type: zigbee_herdsman_1.Zcl.DataType.ENUM8 },
                    alarmSoundEnable: { name: "alarmSoundEnable", ID: 0x2026, type: zigbee_herdsman_1.Zcl.DataType.BOOLEAN, write: true },
                    alarmLightEnable: { name: "alarmLightEnable", ID: 0x2022, type: zigbee_herdsman_1.Zcl.DataType.BOOLEAN, write: true },
                    alarmSoundType: { name: "alarmSoundType", ID: 0x2023, type: zigbee_herdsman_1.Zcl.DataType.ENUM8, write: true },
                    alarmVolumeLevel: { name: "alarmVolumeLevel", ID: 0x2024, type: zigbee_herdsman_1.Zcl.DataType.ENUM8, write: true },
                    alarmDuration: { name: "alarmDuration", ID: 0x2025, type: zigbee_herdsman_1.Zcl.DataType.UINT16, write: true },
                    spilt: { name: "spilt", ID: 0x2000, type: zigbee_herdsman_1.Zcl.DataType.UINT8, write: true },
                },
                commands: {
                    alertCommand: { name: "alertCommand", ID: 0x0f, parameters: [{ name: "data", type: zigbee_herdsman_1.Zcl.BuffaloZclDataType.LIST_UINT8 }] },
                },
                commandsResponse: {},
            }),
        ...
        configure: async (device, coordinatorEndpoint) => {
            ...
            await endpoint.read("customClusterEwelink", ["spilt"], manufacturerOptions);
        },
    },
    {
        ...
        extend: [
            ...
            m.numeric({
                ...
                cluster: "msOccupancySensing",
                attribute: { ID: 0x0010, type: zigbee_herdsman_1.Zcl.DataType.UINT16 },
    ...
    },
    {
        ...
        extend: [
            m.deviceAddCustomCluster("customClusterEwelink", {
                ...
                ID: 0xfc11,
                attributes: {
                    comfortTemperatureMax: { name: "comfortTemperatureMax", ID: 0x0003, type: zigbee_herdsman_1.Zcl.DataType.INT16, write: true },
                    comfortTemperatureMin: { name: "comfortTemperatureMin", ID: 0x0004, type: zigbee_herdsman_1.Zcl.DataType.INT16, write: true },
                    comfortHumidityMin: { name: "comfortHumidityMin", ID: 0x0005, type: zigbee_herdsman_1.Zcl.DataType.UINT16, write: true },
                    comfortHumidityMax: { name: "comfortHumidityMax", ID: 0x0006, type: zigbee_herdsman_1.Zcl.DataType.UINT16, write: true },
                    temperatureUnits: { name: "temperatureUnits", ID: 0x0007, type: zigbee_herdsman_1.Zcl.DataType.UINT16, write: true },
                    temperatureCalibration: { name: "temperatureCalibration", ID: 0x2003, type: zigbee_herdsman_1.Zcl.DataType.INT16, write: true },
                    humidityCalibration: { name: "humidityCalibration", ID: 0x2004, type: zigbee_herdsman_1.Zcl.DataType.INT16, write: true },
                },
                commands: {},
    ...
    },
    {
        ...
        extend: [
            ...
            m.binary({
                ...
                cluster: "customClusterEwelink",
                attribute: "outlet_control_protect",
                description: "When enabled, the device turns off immediately when the configured threshold is reached. After protection is triggered, it can only be restored manually and cannot be turned on via Z2M.",
                valueOff: [false, 0],
                valueOn: [true, 1],
            }),
            m.binary({
