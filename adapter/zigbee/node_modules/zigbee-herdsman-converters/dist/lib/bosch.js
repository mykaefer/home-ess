...
const NS = "zhc:bosch";
function addWeeklyScheduleExpose(climate) {
    const featureDayOfWeek = new exposes.List("dayofweek", ea.SET, new exposes.Composite("day", "dayofweek", ea.SET).withFeature(new exposes.Enum("day", ea.SET, ["monday", "tuesday", "wednesday", "thursday", "friday", "saturday", "sunday", "away_or_vacation"])))
...
exports.boschBmctExtend = {
    ...
    slzExtends: () => {
        ...
        const reportActionExtend = exports.boschBmctExtend.reportSwitchAction({
...
exports.boschWaterAlarmExtend = {
    waterAlarmCluster: () => m.deviceAddCustomCluster("boschWaterAlarm", {
        ...
        manufacturerCode: exports.manufacturerOptions.manufacturerCode,
        attributes: {
            alarmOnMotion: { name: "alarmOnMotion", ID: 0x0003, type: zigbee_herdsman_1.Zcl.DataType.BOOLEAN, write: true },
        },
        commands: {
            muteAlarmControl: { name: "muteAlarmControl", ID: 0x00, parameters: [{ name: "data", type: zigbee_herdsman_1.Zcl.DataType.UINT8, max: 0xff }] },
            muteAlarmControlResponse: {
                name: "muteAlarmControlResponse",
                ID: 0x01,
                parameters: [{ name: "data", type: zigbee_herdsman_1.Zcl.DataType.ENUM8, max: 0xff }],
            },
        },
        commandsResponse: {},
    }),
    ...
    customIasZoneCluster: () => m.deviceAddCustomCluster("ssIasZone", {
        ...
        commands: {
            alarmControl: {
                ...
                ID: 0x80,
                parameters: [
                    { name: "alarmMode", type: zigbee_herdsman_1.Zcl.DataType.ENUM8, max: 0xff },
                    { name: "alarmTimeout", type: zigbee_herdsman_1.Zcl.DataType.UINT8, max: 0xff },
                ],
            },
        },
        commandsResponse: {},
        ...
        const toZigbee = [
            {
                ...
                convertSet: async (entity, key, value, meta) => {
                    if (key === "manual_smoke_alarm" || key === "manual_burglar_alarm") {
                        ...
                        if (enableAlarm) {
                            const alarmTimer = setTimeout(async () => await sendAlarmControlMessage(entity, broadcastAlarm, alarmMode, timeoutInSeconds), (timeoutInSeconds - 60) * 1000).unref();
...
exports.boschSmartPlugExtend = {
    smartPlugCluster: () => m.deviceAddCustomCluster("boschEnergyDevice", {
        ...
        manufacturerCode: exports.manufacturerOptions.manufacturerCode,
        attributes: {
            autoOffEnabled: { name: "autoOffEnabled", ID: 0x0006, type: zigbee_herdsman_1.Zcl.DataType.BOOLEAN, write: true },
            autoOffTime: { name: "autoOffTime", ID: 0x0007, type: zigbee_herdsman_1.Zcl.DataType.UINT16, write: true, max: 0xffff },
            ledBrightness: { name: "ledBrightness", ID: 0x002c, type: zigbee_herdsman_1.Zcl.DataType.UINT8, write: true, max: 0xff },
            energySavingModeEnabled: { name: "energySavingModeEnabled", ID: 0x002d, type: zigbee_herdsman_1.Zcl.DataType.BOOLEAN, write: true },
            energySavingModeThreshold: { name: "energySavingModeThreshold", ID: 0x002e, type: zigbee_herdsman_1.Zcl.DataType.UINT16, write: true, max: 0xffff },
            energySavingModeTimer: { name: "energySavingModeTimer", ID: 0x002f, type: zigbee_herdsman_1.Zcl.DataType.UINT32, write: true, max: 0xffffffff },
        },
        commands: {},
        commandsResponse: {},
    }),
...
exports.boschThermostatExtend = {
    ...
    relayState: () => {
        ...
        const relayExposes = (relay.exposes ?? []).filter((relayExpose) => typeof relayExpose !== "function");
        const relayEndpoint = (device) => device.endpoints.find((endpoint) => endpoint.supportsInputCluster("genOnOff"));
        const expose = (device) => {
        ...
        const configure = async (device, coordinatorEndpoint) => {
            ...
            if (!endpoint) {
                logger_1.logger.debug(`Skipping Bosch thermostat relay reporting for ${device.ieeeAddr}: no genOnOff input cluster`, NS);
                return;
            }
            await endpoint.bind("genOnOff", coordinatorEndpoint);
            await endpoint.configureReporting("genOnOff", (0, reporting_1.payload)("onOff", 0, constants_1.repInterval.MAX, 1));
        };
