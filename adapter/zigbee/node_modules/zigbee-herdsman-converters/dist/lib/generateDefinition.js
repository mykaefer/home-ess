...
async function extenderElectricityMeter(device, endpoints) {
    ...
    if (args.cluster === "electrical") {
        ...
        const measurementType = await (0, utils_1.getClusterAttributeValue)(endpoint, "haElectricalMeasurement", "measurementType", 0);
...
async function extenderBinaryInput(device, endpoints) {
    ...
    for (const endpoint of endpoints) {
        ...
        const name = await (0, utils_1.getClusterAttributeValue)(endpoint, "genBinaryInput", "description", endpointName ? "binary_input" : `binary_input_${endpoint.ID}`);
        let label;
...
async function extenderBinaryOutput(device, endpoints) {
    ...
    for (const endpoint of endpoints) {
        ...
        const name = await (0, utils_1.getClusterAttributeValue)(endpoint, "genBinaryOutput", "description", endpointName ? "binary_output" : `binary_output_${endpoint.ID}`);
        let label;
...
function getUnitfromApplicationType(applicationType) {
...
function getUnitfromBACnetUnit(bacnetUnit) {
    const bacnetUnitMap = {
        0: "m²",
        1: "ft²",
        2: "mA",
...
async function extenderAnalogInput(device, endpoints) {
    const generated = [];
    let endpointNames;
    for (const endpoint of endpoints) {
        if (!onlyFirstDeviceEnpoint(device, endpoints)) {
        ...
        const name = await (0, utils_1.getClusterAttributeValue)(endpoint, "genAnalogInput", "description", endpointNames ? "analog_input" : `analog_input_${endpoint.ID}`);
        let label;
        if (name !== "analog_input" && name !== `analog_input_${endpoint.ID}`) {
            label = name;
        }
        const description = `Analog Input ${name} on endpoint ${endpoint.ID}`;
        const applicationType = await (0, utils_1.getClusterAttributeValue)(endpoint, "genAnalogInput", "applicationType", undefined);
        let unit;
        if (applicationType !== undefined) {
            unit = getUnitfromApplicationType(applicationType);
        }
        if (unit === undefined) {
            const bacnet_unit = await (0, utils_1.getClusterAttributeValue)(endpoint, "genAnalogInput", "engineeringUnits", undefined);
        ...
        const args = {
            name: name.replace(/\s+/g, "_").toLowerCase(),
            label,
            valueMin: await (0, utils_1.getClusterAttributeValue)(endpoint, "genAnalogInput", "minPresentValue", undefined),
            valueMax: await (0, utils_1.getClusterAttributeValue)(endpoint, "genAnalogInput", "maxPresentValue", undefined),
            valueStep: await (0, utils_1.getClusterAttributeValue)(endpoint, "genAnalogInput", "resolution", undefined),
...
async function extenderAnalogOutput(device, endpoints) {
    const generated = [];
    let endpointNames;
    for (const endpoint of endpoints) {
        if (!onlyFirstDeviceEnpoint(device, endpoints)) {
        ...
        const name = await (0, utils_1.getClusterAttributeValue)(endpoint, "genAnalogOutput", "description", endpointNames ? "analog_output" : `analog_output_${endpoint.ID}`);
        let label;
        if (name !== "analog_output" && name !== `analog_output_${endpoint.ID}`) {
            label = name;
        }
        const description = `Analog Output ${name} on endpoint ${endpoint.ID}`;
        const applicationType = await (0, utils_1.getClusterAttributeValue)(endpoint, "genAnalogOutput", "applicationType", undefined);
        let unit;
        if (applicationType !== undefined) {
            unit = getUnitfromApplicationType(applicationType);
        }
        if (unit === undefined) {
            const bacnet_unit = await (0, utils_1.getClusterAttributeValue)(endpoint, "genAnalogOutput", "engineeringUnits", undefined);
            if (bacnet_unit !== undefined) {
                unit = getUnitfromBACnetUnit(bacnet_unit);
            }
        }
        const args = {
            name: name.replace(/\s+/g, "_").toLowerCase(),
            label,
            valueMin: await (0, utils_1.getClusterAttributeValue)(endpoint, "genAnalogOutput", "minPresentValue", undefined),
            valueMax: await (0, utils_1.getClusterAttributeValue)(endpoint, "genAnalogOutput", "maxPresentValue", undefined),
            valueStep: await (0, utils_1.getClusterAttributeValue)(endpoint, "genAnalogOutput", "resolution", undefined),
