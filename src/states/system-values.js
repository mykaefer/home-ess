...
// Provider der von homeESS berechneten System-States (Leistungen, Erträge,
// Summen, Prognosen und Betriebszustände). Jeder Eintrag liefert:
...
} = require('../photovoltaik/sun-intensity');
...
async function buildCalculatedInternalValues(db, cache) {
  ...
  const [pv, strom, sunIntensity, sunIntensitySample, forecast, prognosisConfig] = await Promise.all([
    readPhotovoltaikValues(db, cache, plants),
    readStromverbrauchValues(db, cache),
    readSunIntensityAverages(db),
    computeSunIntensitySample(db, cache),
  ...
  const sunIntensityNow = sunIntensitySample == null ? null : sunIntensitySample.intensity;
  const brightnessNow = computeBrightnessPercent(mqttConfig, cache, sunIntensitySample);
  ...
  entries.push(percentEntry('prognose.helligkeit', 'Helligkeit aktuell', brightnessNow));
...
let calculatedInFlight = null;

async function listCalculatedInternalValues(db, cache) {
  if (calculatedCache && calculatedCache.db === db && calculatedCache.cache === cache &&
      Date.now() - calculatedCache.at < SNAPSHOT_CACHE_MS) {
    metrics.counter('internalValues.calculatedCacheHit');
    return calculatedCache.values;
  }
  if (calculatedInFlight && calculatedInFlight.db === db && calculatedInFlight.cache === cache) {
    metrics.counter('internalValues.calculatedShared');
    return calculatedInFlight.promise;
  }
  const promise = metrics.measure(
    'internalValues.calculatedBuild',
    () => buildCalculatedInternalValues(db, cache)
  );
  calculatedInFlight = { db, cache, promise };
  try {
    const values = await promise;
    calculatedCache = { db, cache, at: Date.now(), values };
    return values;
  } finally {
    if (calculatedInFlight && calculatedInFlight.promise === promise) calculatedInFlight = null;
  }
}

function invalidateInternalValues() {
  calculatedCache = null;
}
