...
const {
  ...
  validSensorMessage, validFingerprintMessage,
...
function fingerprintManifest() {
  const result = binaryManifest('fingerprint-event-v1');
  result.device_types = [...result.device_types, 'fingerprint_reader'];
  Object.assign(result.features, {
    fingerprint_recognition: true, fingerprint_enrollment: true,
  });
  Object.assign(result.hardware_capabilities, {
    fingerprint_uart_baud: 57600,
    fingerprint_wakeup_optional: true,
    fingerprint_led_effects: ['breathing', 'flashing', 'on', 'off', 'gradual_on', 'gradual_off'],
    fingerprint_led_colors: ['red', 'blue', 'purple'],
  });
  result.limits.maximum_fingerprint_templates = 256;
  return result;
}

function fingerprintConfig(revision = 8) {
  return {
    revision, device_type: 'fingerprint_reader',
    uart: { rx_pin: 13, tx_pin: 15 }, wakeup_pin: 12,
    led: {
...
test('fingerprint-event-v1 validiert R503-Hardware, LED-Szenen und Ereignisse', () => {
  const manifest = fingerprintManifest();
  assert.equal(validateManifestInfo(manifest), manifest);
  assert.deepEqual(validation.validateHardwareConfig(fingerprintConfig(), manifest), fingerprintConfig());
  assert.throws(() => validation.validateHardwareConfig({
    ...fingerprintConfig(), uart: { rx_pin: 16, tx_pin: 15 },
  }, manifest), /GPIO 16/);
  assert.throws(() => validation.validateHardwareConfig({
    ...fingerprintConfig(), pins: [{ pin: 13, direction: 'output' }],
  }, manifest), /überlappt/);
  assert.throws(() => validation.validateHardwareConfig({
    ...fingerprintConfig(), led: {
      ...fingerprintConfig().led,
      success: { effect: 'rainbow', color: 'blue', speed: 32, count: 2 },
    },
  }, manifest), /LED-Effekt/);

  const envelope = (type, payload) => ({ type, payload });
  assert.equal(validFingerprintMessage(envelope('fingerprint.status', {
    config_revision: 8, reply_to: null, online: true, enrolling: false,
    capacity: 200, template_count: 2, occupied_slots: [1, 7], last_error: null,
  })), true);
  assert.equal(validFingerprintMessage(envelope('fingerprint.match', {
    config_revision: 8, event_sequence: 1, occurred_at_uptime_milliseconds: 100,
    template_id: 7, confidence: 143,
  })), true);
  assert.equal(validFingerprintMessage(envelope('fingerprint.command.accepted', {
    reply_to: 'adapter-1', action: 'enroll', template_id: 7,
  })), true);
  assert.equal(validFingerprintMessage(envelope('fingerprint.status', {
    config_revision: 8, reply_to: null, online: true, enrolling: false,
    capacity: 2, template_count: 2, occupied_slots: [1, 1], last_error: null,
  })), false);
  ...
  assert.deepEqual(helpers.normalizeFingerprintBindings({ fingerprint: {
    ...
    7: { name: 'Daumen', topic: 'mqtt://main/door', action: 'counter', set_value: true, counter_step: -1 },
  });
});
...
test('Sensorprofil validiert gemeinsame Busse, freie Binary-Pins und Messwerte', () => {
  const { sensorUiFields } = createHdpAdapter._test;
  assert.deepEqual(sensorUiFields('dht22'), ['identity', 'interval', 'data_gpio']);
  assert.deepEqual(sensorUiFields('bme280'), ['identity', 'interval', 'i2c']);
  assert.deepEqual(sensorUiFields('hx711'), [
    'identity', 'interval', 'data_gpio', 'clock_gpio', 'calibration',
  ]);
  assert.deepEqual(sensorUiFields('analog'), [
    'identity', 'interval', 'adc', 'calibration',
  ]);
  assert.deepEqual(sensorUiFields('unbekannt'), []);
  const manifest = binaryManifest('sensor-reading-v1');
  manifest.device_types.push('sensors');
  manifest.features.sensor_reading = true;
  manifest.hardware_capabilities.sensor_types = validation.SENSOR_TYPES;
  manifest.limits.maximum_sensors = 8;
  manifest.limits.minimum_sensor_interval_milliseconds = 100;
  assert.equal(validateManifestInfo(manifest).runtime_profile, 'sensor-reading-v1');
  const config = validation.validateHardwareConfig({
    revision: 7, device_type: 'sensors',
    sensors: [
      { sensor_id: 'climate', sensor_type: 'bme280', sample_interval_milliseconds: 5000, sda_pin: 4, scl_pin: 5, address: 0x76 },
      { sensor_id: 'light', sensor_type: 'bh1750', sample_interval_milliseconds: 1000, sda_pin: 4, scl_pin: 5, address: 0x23 },
      { sensor_id: 'weight', sensor_type: 'hx711', sample_interval_milliseconds: 500, data_pin: 12, clock_pin: 13 },
    ],
    pins: [{ pin: 14, direction: 'input', input_type: 'button' }],
  }, manifest);
  assert.equal(config.sensors.length, 3);
  assert.equal(config.pins[0].pin, 14);
  assert.throws(() => validation.validateHardwareConfig({
    ...config, pins: [{ pin: 4, direction: 'output' }],
  }, manifest), /überlappt/);
  assert.throws(() => validation.validateHardwareConfig({
    ...config,
    sensors: config.sensors.map((sensor) => sensor.sensor_id === 'weight'
      ? { ...sensor, data_pin: 4 } : sensor),
  }, manifest), /überlappt/);
  const message = {
    type: 'sensor.sample', payload: {
      config_revision: 7, sensor_id: 'climate', sensor_type: 'bme280',
      sample_sequence: 1, captured_at_uptime_milliseconds: 1000,
      status: 'ok', error: null,
      values: { temperature_millicelsius: 22430, pressure_pascal: 100842 },
    },
  };
  assert.equal(validSensorMessage(message), true);
});

test('Sensorwerte werden in SI-Einheiten und kalibrierte States umgesetzt', () => {
  const device = {
    deviceId: DEVICE_ID, hardwareConfig: {
      revision: 1, device_type: 'sensors', sensors: [
        { sensor_id: 'scale', sensor_type: 'hx711', sample_interval_milliseconds: 500 },
      ], pins: [],
    },
    sensorCalibration: { scale: { scale: 0.25, offset: -10 } },
    sensorSamples: {}, bindings: createHdpAdapter._test.defaultBindings(),
  };
  createHdpAdapter._test.applySensorSample(device, {
    sensor_id: 'scale', sensor_type: 'hx711', sample_sequence: 2,
    captured_at_uptime_milliseconds: 1234, status: 'ok', error: null,
    values: { raw: 1000 },
  });
  assert.equal(device.sensorSamples.scale.values.raw, 240);
  const states = createHdpAdapter._test.deviceStateValues(device);
  assert.equal(states.find((entry) => entry.address.endsWith('/raw')).value, 240);
