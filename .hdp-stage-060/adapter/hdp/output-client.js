...
class OutputClient extends EventEmitter {
  constructor(options) {
    ...
    this.runtimeBrightness = new Map();
  }
  ...
  async setRuntimeBrightness(outputId, percent) {
    if (this.manifest.features.runtime_brightness !== true) {
      throw Object.assign(new Error('Manifest unterstützt keine Laufzeithelligkeit.'), {
        code: 'UNSUPPORTED_MESSAGE_TYPE',
      });
    }
    if (!Number.isInteger(percent) || percent < 0 || percent > 100) {
      throw Object.assign(new Error('Laufzeithelligkeit muss zwischen 0 und 100 liegen.'), {
        code: 'INVALID_REQUEST',
      });
    }
    if (this.runtimeBrightness.get(outputId) === percent) return { unchanged: true };
    const { config } = this.output(outputId);
    const response = await this.requestRecovered('output.brightness.set', {
      output_id: outputId,
      config_revision: config.revision,
      brightness_percent: percent,
    }, 5000);
    if (!response || response.type !== 'output.brightness.applied' || !response.payload
        || response.payload.output_id !== outputId
        || response.payload.config_revision !== config.revision
        || response.payload.brightness_percent !== percent
        || !Number.isInteger(response.payload.applied_at_uptime_milliseconds)) {
      throw Object.assign(new Error('Ungültige Helligkeitsbestätigung.'), {
        code: 'INVALID_OUTPUT_STATE',
      });
    }
    this.runtimeBrightness.set(outputId, percent);
  ...
  async play(outputId, timelineId, loop = true, startDelay = 0) {
    ...
    // Never restart an already matching loop merely because play() was queued
    // twice or controller state was reconciled. device.status is telemetry;
    // only a genuinely different output state authorizes a new play command.
    const current = await this.status(outputId);
    if (['timeline_scheduled', 'timeline_playing'].includes(current.mode)
        && current.timeline_id === timelineId && current.loop === loop) {
      this.baselines.add(outputId);
      this.frames.delete(outputId);
